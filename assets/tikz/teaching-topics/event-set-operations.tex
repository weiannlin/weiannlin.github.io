...
% frame + circles + region labels for the two-circle panels
\newcommand{\twocircleframe}{%
  ...
  \node[journalink, font=\small] at (-0.53,0) {$A\cap B^{\prime}$};
  \node[journalink, font=\small] at (1.73,0) {$A^{\prime}\cap B$};
...
\begin{tikzpicture}[x=1cm, y=1cm, every node/.style={font=\normalsize}, accentfill/.style={fill=journalaccent, fill opacity=0.15}]
  ...
  \begin{scope}[shift={(5.2,-4.6)}]
    ...
    \node[journalink] at (2.0,0) {$A^{\prime}$};
    \panelcaption{$A^{\prime}$}
    \spacelabel
  \end{scope}
