...
topic: 16
order: 416
permalink: /teaching-topics/ch4-p16-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch4-p15-candidate/)由[貝塔函數](/teaching-topics/ch4-p15-candidate/#def-beta-function)出發，給出[貝塔分配](/teaching-topics/ch4-p15-candidate/#def-beta-distribution)的定義。貝塔函數本身就是由兩個[伽瑪函數](/teaching-topics/ch4-p10-candidate/#def-gamma-function)的乘積除以另一個伽瑪函數而來，因此貝塔分配與[伽瑪分配](/teaching-topics/ch4-p12-candidate/#def-gamma-distribution)之間存在一項直接的關係，本篇處理的就是這項關係。

本篇先以 Jacobian 法證明: 兩個比例參數相同且彼此獨立的伽瑪變數，其中一個在兩者之和中所佔的比例服從貝塔分配，兩者之和則仍為伽瑪分配，而且這兩個新變數彼此獨立。接著說明這項關係背後的想法，以及它在[指數分配](/teaching-topics/ch4-p10-candidate/#def-exponential-distribution)上的特例。最後以三道例題作為演練，其中兩道涉及[標準均勻分配](/teaching-topics/ch4-p14-candidate/#def-uniform-distribution)的順序統計量 <span lang="en">(order statistic)</span>。
...
<!-- ref-point: 待第三章第 24 篇 (多對多函數轉換，其中的 Jacobian 法見書稿 mathstatch3.tex
     第 4360 行的方框，該處沒有 theorem 環境也沒有 label，回填時連向該篇的相應小節) 發布後，
     將下面證明中的「Jacobian 法」改為指向該處的站內連結。
     本篇另有兩處同樣待回填，見 Example 4.41 第 (3) 小題與 Example 4.42 第 (2) 小題。 -->

<div class="topic-proof" markdown="1">
**Proof.** 由 Jacobian 法可知，令

<div class="topic-math-layout topic-math-layout--desktop" markdown="1">

$$
U=\frac{X}{\,X+Y\,},\ V=X+Y\ \Longrightarrow\ X=UV,\ Y=V(1-U)
$$
...
$$
\begin{gathered}
U=\frac{X}{\,X+Y\,},\ V=X+Y\\[0.5em]
\Longrightarrow\ X=UV,\ Y=V(1-U)
\end{gathered}
$$
...
&\quad =\frac{1}{\,\beta^{a}\Gamma(a)\,}x^{a-1}e^{-\frac{x}{\beta}}\cdot\frac{1}{\,\beta^{b}\Gamma(b)\,}y^{b-1}e^{-\frac{y}{\beta}}\\[0.45em]
&\quad =\frac{1}{\,\beta^{a+b}\Gamma(a)\Gamma(b)\,}x^{a-1}y^{b-1}e^{-\frac{x+y}{\beta}},\\[0.25em]
&\qquad\qquad x>0,y>0
\end{aligned}
$$
...
\begin{aligned}
&f_{\sssig UV}(u,v)=f_{\sssig XY}\bigl(uv,\ v(1-u)\bigr)\lvert J\rvert\\[0.45em]
&\quad =\frac{1}{\,\beta^{a+b}\Gamma(a)\Gamma(b)\,}(uv)^{a-1}\\[0.25em]
&\qquad \times\bigl[v(1-u)\bigr]^{b-1}e^{-\frac{v}{\beta}}\lvert v\rvert\\[0.45em]
&\quad =\frac{1}{\,\beta^{a+b}\Gamma(a)\Gamma(b)\,}u^{a-1}(1-u)^{b-1}\\[0.25em]
&\qquad \times v^{a+b-1}e^{-\frac{v}{\beta}},\ 0<u<1,v>0
\end{aligned}
$$
...
\begin{aligned}
&f_{\sssig U}(u)\\[0.45em]
&\quad =\int_{0}^{\infty}\frac{1}{\,\beta^{a+b}\Gamma(a)\Gamma(b)\,}u^{a-1}(1-u)^{b-1}\\[0.25em]
&\qquad \times v^{a+b-1}e^{-\frac{v}{\beta}}\,dv\\[0.45em]
&\quad =\frac{1}{\,\beta^{a+b}\Gamma(a)\Gamma(b)\,}u^{a-1}(1-u)^{b-1}\\[0.25em]
&\qquad \times\int_{0}^{\infty}v^{a+b-1}e^{-\frac{v}{\beta}}\,dv\\[0.45em]
&\quad =\frac{1}{\,\beta^{a+b}\Gamma(a)\Gamma(b)\,}u^{a-1}(1-u)^{b-1}\\[0.25em]
&\qquad \times\beta^{a+b}\Gamma(a+b)\\[0.45em]
&\quad =\frac{\Gamma(a+b)}{\,\Gamma(a)\Gamma(b)\,}u^{a-1}(1-u)^{b-1},\ 0<u<1
\end{aligned}
$$
...
\begin{aligned}
&f_{\sssig V}(v)\\[0.45em]
&\quad =\int_{0}^{1}\frac{1}{\,\beta^{a+b}\Gamma(a)\Gamma(b)\,}u^{a-1}(1-u)^{b-1}\\[0.25em]
&\qquad \times v^{a+b-1}e^{-\frac{v}{\beta}}\,du\\[0.45em]
&\quad =\frac{1}{\,\beta^{a+b}\Gamma(a+b)\,}v^{a+b-1}e^{-\frac{v}{\beta}}\\[0.25em]
&\qquad \times\int_{0}^{1}\frac{\Gamma(a+b)}{\,\Gamma(a)\Gamma(b)\,}u^{a-1}(1-u)^{b-1}\,du\\[0.45em]
&\quad =\frac{1}{\,\beta^{a+b}\Gamma(a+b)\,}v^{a+b-1}e^{-\frac{v}{\beta}},\ v>0
\end{aligned}
$$
...
$$
1=\int_{0}^{\infty}\frac{e^{-kx}}{\,k!\,}\,dx=\left[\frac{e^{-kx}}{\,(-k)\cdot k!\,}\right]_{0}^{\infty}=-\frac{1}{\,(-k)\cdot k!\,}\ \Longrightarrow\ k=1
$$
...
&=-\frac{1}{\,(-k)\cdot k!\,}\ \Longrightarrow\ k=1
\end{aligned}
$$
...
&\quad =e^{-x_1}e^{-x_2}=e^{-(x_1+x_2)},\\[0.25em]
...
$$
Y_1=X_1+X_2,\ Y_2=\frac{X_1}{\,X_1+X_2\,}\ \Longrightarrow\ X_1=Y_1Y_2,\ X_2=Y_1(1-Y_2)
$$
...
$$
\begin{gathered}
Y_1=X_1+X_2,\ Y_2=\frac{X_1}{\,X_1+X_2\,}\\[0.5em]
\Longrightarrow\ X_1=Y_1Y_2,\ X_2=Y_1(1-Y_2)
\end{gathered}
$$
...
&\quad =f_{\sssig X_1X_2}\bigl(y_1y_2,\ y_1(1-y_2)\bigr)\lvert\mathbf{J}\rvert\\[0.45em]
&\quad =e^{-y_1}\lvert-y_1\rvert=y_1e^{-y_1},\\[0.25em]
...
<!-- ref-point: 待第三章第 28 篇 (順序統計量的抽樣分配，書稿 mathstatch3.tex 第 5232 行的
     Theorem 3.24，anchor 為 #thm-order-stat-samp-dist-pdf) 發布後，將下面 Example 4.42
     第 (1) 小題的「順序統計量」改為指向該 anchor 的站內連結。
     另有兩處指向同一篇但不是指向該定理: Example 4.42 之後的 Note 與 Example 4.43 第 (1)
     小題所用的「順序統計量的性質」，對應書稿 mathstatch3.tex 第 5391 行 uniformOrder 標記
     之後的標準均勻分配順序統計量各項貝塔分配結果 (該處沒有環境也沒有可用的 anchor，回填時
     連向第三章第 28 篇的相應小節)。 -->
...
(1) 由順序統計量，我們可知 $Y_{\sssig (r)}$ 與 $Y_{\sssig (s)}$ 之 joint pdf 為
{: .topic-paren-item}
...
$$
\begin{aligned}
&f_{Y_{\sssig (r)}Y_{\sssig (s)}}(u,v)\\[0.45em]
&\quad =\frac{n!}{\,(r-1)!(s-r-1)!(n-s)!\,}\\[0.25em]
&\qquad\qquad \times f_{\sssig Y}(u)f_{\sssig Y}(v)\bigl[F_{\sssig Y}(u)\bigr]^{r-1}\\[0.25em]
&\qquad\qquad \times\bigl[F_{\sssig Y}(v)-F_{\sssig Y}(u)\bigr]^{s-r-1}\\[0.25em]
&\qquad\qquad \times\bigl[1-F_{\sssig Y}(v)\bigr]^{n-s},\\[0.25em]
&\qquad\qquad 0<u\leqslant v<1\\[0.45em]
&\quad =\frac{n!}{\,(r-1)!(s-r-1)!(n-s)!\,}\\[0.25em]
&\qquad\qquad \times u^{r-1}(v-u)^{s-r-1}(1-v)^{n-s},\\[0.25em]
&\qquad\qquad 0<u\leqslant v<1
\end{aligned}
$$
...
$$
\begin{aligned}
&\mathbb{E}\bigl(Y_{\sssig (r)}Y_{\sssig (s)}\bigr)\\[0.25em]
&\quad =\int_{0}^{1}\int_{0}^{v}uv\,f_{Y_{\sssig (r)}Y_{\sssig (s)}}(u,v)\,du\,dv\\[0.25em]
&\quad =\frac{\,(s+1)r\,}{\,(n+2)(n+1)\,}\\[0.7em]
&\mathbb{E}\bigl(Y_{\sssig (r)}\bigr)=\int_{0}^{1}\int_{0}^{v}u\,f_{Y_{\sssig (r)}Y_{\sssig (s)}}(u,v)\,du\,dv\\[0.25em]
&\quad =\frac{r}{\,n+1\,}\\[0.7em]
&\mathbb{E}\bigl(Y_{\sssig (s)}\bigr)=\int_{0}^{1}\int_{0}^{v}v\,f_{Y_{\sssig (r)}Y_{\sssig (s)}}(u,v)\,du\,dv\\[0.25em]
&\quad =\frac{s}{\,n+1\,}\\[0.7em]
&\mathrm{Cov}\bigl(Y_{\sssig (r)},Y_{\sssig (s)}\bigr)\\[0.25em]
&\quad =\mathbb{E}\bigl(Y_{\sssig (r)}Y_{\sssig (s)}\bigr)-\mathbb{E}\bigl(Y_{\sssig (r)}\bigr)\mathbb{E}\bigl(Y_{\sssig (s)}\bigr)\\[0.25em]
&\quad =\frac{r(n+1-s)}{\,(n+1)^2(n+2)\,},\\[0.25em]
...
&\quad =\frac{n!}{\,(n-2)!\,}r^{n-2},\ 0<t\leqslant r+t<1
\end{aligned}
$$
...
&\quad =\frac{n!}{\,(n-2)!\,}r^{n-2}(1-r),\ 0<r<1
\end{aligned}
$$
...
(3) 經由[機率積分轉換](/teaching-topics/ch4-p14-candidate/#thm-p-i-t)可知
{: .topic-paren-item}
...
事實上，這個問題並不需要如此繁複的做法，我們可以利用順序統計量的性質來處理這個問題，作法如下:
...
$$
\begin{aligned}
&\mathrm{Var}\bigl(Y_{\sssig (s)}-Y_{\sssig (r)}\bigr)\\[0.45em]
&\quad =\mathrm{Var}\bigl(Y_{\sssig (s)}\bigr)+\mathrm{Var}\bigl(Y_{\sssig (r)}\bigr)\\[0.25em]
&\qquad\qquad -2\,\mathrm{Cov}\bigl(Y_{\sssig (s)},Y_{\sssig (r)}\bigr)
\end{aligned}
$$
...
$$
\begin{aligned}
&\mathbb{E}\left[\frac{1}{\,2\,}(Y_1+Y_3)\right]\\[0.45em]
&\quad =\frac{1}{\,2\,}\mathbb{E}(Y_1)+\frac{1}{\,2\,}\mathbb{E}(Y_3)=\frac{1}{\,2\,}
\end{aligned}
$$
...
&\quad =\frac{1}{\,4\,}\mathrm{Var}(Y_1)+\frac{1}{\,4\,}\mathrm{Var}(Y_3)\\[0.25em]
&\qquad\qquad +\frac{2}{\,2\times 2\,}\mathrm{Cov}(Y_1,Y_3)\\[0.45em]
&\quad =\frac{1}{\,40\,}
\end{aligned}
$$
...
[Theorem 4.19](#thm-gamma-to-beta) 把[伽瑪分配](/teaching-topics/ch4-p12-candidate/#def-gamma-distribution)與[貝塔分配](/teaching-topics/ch4-p15-candidate/#def-beta-distribution)接了起來: 兩個比例參數同為 $\beta$ 而形狀參數分別為 $a$ 與 $b$ 的獨立伽瑪變數，其中一個在兩者之和中所佔的比例 $U=\frac{X}{\,X+Y\,}$ 服從 <span class="text-nowrap">$\mathrm{Beta}(\alpha=a,\ \beta=b)$，</span>兩者之和 $V=X+Y$ 則服從 <span class="text-nowrap">$\mathrm{Gamma}(\alpha=a+b,\ \beta)$，</span>而且 $U$ 與 $V$ 彼此獨立。證明採 Jacobian 法，先寫出兩個伽瑪變數的聯合機率密度函數，換元之後所得到的聯合機率密度函數恰好可以拆成只含 $u$ 的一段與只含 $v$ 的一段，兩段各自積分之後，貝塔分配與伽瑪分配的機率函數就分別出現在 $U$ 與 $V$ 的邊際機率密度函數上。

定理之後的說明給出一個稍不嚴謹但簡單直接的想法: 比例的平均是兩個期望值之間的比例，也就是 $\frac{a\beta}{\,a\beta+b\beta\,}=\frac{a}{\,a+b\,}$ 這個比值，恰好是貝塔分配的期望值。同一段也指出一個特例，即兩個比例參數相同且彼此獨立的[指數分配](/teaching-topics/ch4-p10-candidate/#def-exponential-distribution)變數，其比例服從[標準均勻分配](/teaching-topics/ch4-p14-candidate/#def-uniform-distribution)。
...
[下一篇](/teaching-topics/ch4-p17-candidate/)轉入常態相關的機率模型，先給出高斯積分，再定義常態分配，並說明標準化與線性組合的可加性。
