...
topic: 11
order: 211
permalink: /teaching-topics/ch2-p211-candidate/
date: 2026-08-06
published: false
...
[上一篇](/teaching-topics/ch2-p210-candidate/)介紹眾數，也就是使機率函數在值域的閉包上取到最大值的那些點。眾數關心的是一個分配的哪一處最可能發生，但要指出一個分配的中央位置，還有另一種常見的作法: 找一個點，使它左右兩側的機率各佔一半。

本篇介紹的中位數就是這樣的量數。以下先給出定義，接著說明中位數可能不唯一、如何由累積分配函數找到其中一個，以及它把分配切成前後兩段的直觀；然後介紹一個與期望值恰成對照的性質: [期望值使平方離差的期望值達到最小](/teaching-topics/ch2-p209-candidate/#thm-mean-minimizes-squared-deviation)，中位數則使絕對離差的期望值達到最小，並附上證明與逐步的說明；最後以一張圖說明中位數的直觀意涵。
...
與[眾數](/teaching-topics/ch2-p210-candidate/#def-mode)相同，中位數亦可能不唯一，或甚至不在值域內。
...
(3) 直觀上來說，中位數的意義在於**該點將整個分配分成前後兩半**，這套定義與直觀，能夠直接推廣至[**分位數 <span lang="en">(quantile)</span>**](/teaching-topics/ch2-p212-candidate/#def-quantile) 上，我們稍後會談到。
{: .topic-paren-item}
...
在 [Theorem 2.14](/teaching-topics/ch2-p209-candidate/#thm-mean-minimizes-squared-deviation) 中我們曾提過，$\mathbb{E}\bigl[(X-a)^{2}\bigr]$ 是一種 $X$ 與 $a$ 之間的平均離散程度，但離散程度未必要使用平方作為考量。某些時候我們會使用 $X$ 和 $a$ 的**距離**，也就是 $\lvert X-a\rvert$，作為離散程度的依據，此時我們會有以下定理。
...
**Proof.** 由 [Definition 2.10](#def-median) 與絕對值的分段寫法可知
...
&\quad=\int_{-\infty}^{\infty}\lvert x-a\rvert\,f_{\sssig X}(x)\,dx\\[0.45em]
&\quad=\int_{-\infty}^{a}(a-x)\,f_{\sssig X}(x)\,dx\\[0.2em]
&\qquad+\int_{a}^{\infty}(x-a)\,f_{\sssig X}(x)\,dx\\[0.45em]
&\quad=\int_{-\infty}^{\eta_{\sssig X}}(a-x)\,f_{\sssig X}(x)\,dx\\[0.2em]
...
&\quad=\int_{-\infty}^{\eta_{\sssig X}}(\eta_{\sssig X}-x)\,f_{\sssig X}(x)\,dx\\[0.2em]
&\qquad+\int_{\eta_{\sssig X}}^{\infty}(x-\eta_{\sssig X})\,f_{\sssig X}(x)\,dx\\[0.2em]
&\qquad+2\int_{a}^{\eta_{\sssig X}}(x-a)\,f_{\sssig X}(x)\,dx\\[0.45em]
&\quad=\mathbb{E}\bigl[\lvert X-\eta_{\sssig X}\rvert\bigr]\\[0.2em]
&\qquad+2\int_{a}^{\eta_{\sssig X}}(x-a)\,f_{\sssig X}(x)\,dx\\[0.45em]
&\quad\geqslant\mathbb{E}\bigl[\lvert X-\eta_{\sssig X}\rvert\bigr]=g(\eta_{\sssig X})
\end{aligned}
$$
...
&\quad=-\int_{a}^{\eta_{\sssig X}}(a-x)\,f_{\sssig X}(x)\,dx\\[0.45em]
&\quad=\int_{a}^{\eta_{\sssig X}}(x-a)\,f_{\sssig X}(x)\,dx
\end{aligned}
$$
...
&\quad=a\int_{-\infty}^{\eta_{\sssig X}}f_{\sssig X}(x)\,dx\\[0.2em]
&\qquad-\int_{-\infty}^{\eta_{\sssig X}}x\,f_{\sssig X}(x)\,dx\\[0.45em]
&\quad=a\cdot\frac{1}{2}-\int_{-\infty}^{\eta_{\sssig X}}x\,f_{\sssig X}(x)\,dx
\end{aligned}
$$
...
&\quad=\int_{\eta_{\sssig X}}^{\infty}x\,f_{\sssig X}(x)\,dx\\[0.2em]
&\qquad-a\int_{\eta_{\sssig X}}^{\infty}f_{\sssig X}(x)\,dx\\[0.45em]
&\quad=\int_{\eta_{\sssig X}}^{\infty}x\,f_{\sssig X}(x)\,dx-a\cdot\frac{1}{2}
\end{aligned}
$$
...
&\qquad+\int_{\eta_{\sssig X}}^{\infty}(x-a)\,f_{\sssig X}(x)\,dx\\[0.45em]
&\quad=\int_{\eta_{\sssig X}}^{\infty}x\,f_{\sssig X}(x)\,dx\\[0.2em]
&\qquad-\int_{-\infty}^{\eta_{\sssig X}}x\,f_{\sssig X}(x)\,dx
\end{aligned}
$$
...
$$
\begin{aligned}
&\int_{-\infty}^{\eta_{\sssig X}}\eta_{\sssig X}\,f_{\sssig X}(x)\,dx\\[0.2em]
&\qquad-\int_{\eta_{\sssig X}}^{\infty}\eta_{\sssig X}\,f_{\sssig X}(x)\,dx\\[0.45em]
&\quad=\eta_{\sssig X}\int_{-\infty}^{\eta_{\sssig X}}f_{\sssig X}(x)\,dx\\[0.2em]
&\qquad-\eta_{\sssig X}\int_{\eta_{\sssig X}}^{\infty}f_{\sssig X}(x)\,dx=0
\end{aligned}
$$
...
&\quad=2\int_{\eta_{\sssig X}}^{a}(a-x)\,f_{\sssig X}(x)\,dx\geqslant0
\end{aligned}
$$
...
&\qquad+2\int_{a}^{\eta_{\sssig X}}(x-a)\,f_{\sssig X}(x)\,dx\\[0.45em]
&\quad\geqslant\mathbb{E}\bigl[\lvert X-\eta_{\sssig X}\rvert\bigr]\\[0.2em]
&\qquad=g(\eta_{\sssig X}),\quad\forall a\in\mathbb{R}
\end{aligned}
$$
...
<figure id="fig-median-intuition" class="topic-figure topic-figure--wide">
  ...
  <figcaption><span class="topic-figure__label">Fig. 2.14.</span> 中位數 $\eta_{\sssig X}$ 把密度曲線下方的面積切成相等的兩塊，左右各為 $\frac{1}{2}$，圖中以陰影標出左邊那一塊。所繪為伽瑪分配 $\mathrm{Gamma}(4,1.7)$ 的密度，其中位數 <span class="text-nowrap">$\fallingdotseq2.160$；</span>曲線只畫到 $x=6.4$ 為止，右邊那一塊實際畫出的面積因而 $\fallingdotseq0.4946$，短少的部分 $\fallingdotseq0.005380$，落在 <span class="text-nowrap">$x>6.4$ 的右尾。</span></figcaption>
</figure>
...
[Theorem 2.17](#thm-median-minimizes-absolute-deviation) 與 [Theorem 2.14](/teaching-topics/ch2-p209-candidate/#thm-mean-minimizes-squared-deviation) 恰成對照: 平均離散程度若以平方衡量，使它最小的是期望值；若改以距離衡量，使 $\mathbb{E}\bigl[\lvert X-a\rvert\bigr]$ 最小的則是中位數。證明的作法是把 $\lvert x-a\rvert$ 的積分依 $a$ 與 $\eta_{\sssig X}$ 拆成四段，合併之後餘下一個非負的項，該項在 $a=\eta_{\sssig X}$ 時為零。[Fig. 2.14](#fig-median-intuition) 則畫出了中位數把密度曲線下方的面積切成相等兩塊的樣子。

[下一篇](/teaching-topics/ch2-p212-candidate/)接著把中位數推廣為分位數，其中的 [Example 2.26](/teaching-topics/ch2-p212-candidate/#ex-nonunique-median) 會給出一個中位數不唯一的具體例子。
