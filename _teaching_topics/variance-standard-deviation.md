...
<figure class="topic-figure topic-figure--wide">
  ...
  <figcaption><span class="topic-figure__label">Fig. 2.14.</span> 兩個分配可以有相同的期望值，卻有不同的分散程度。紅色曲線較分散，綠色曲線較集中；前者對應的變異數較大。</figcaption>
</figure>
...
平移不會改變分散程度。若把每個取值都加上同一個常數 $b$，整個分配只是往左或往右移動，離平均值的相對距離並未改變。
...
變異數適合代數運算，標準差則把單位還原到與隨機變數相同的尺度。[下一篇文章](/teaching-topics/mixed-random-variables/)會回到分配型態，討論同時具有單點機率與連續密度的混合型隨機變數。
