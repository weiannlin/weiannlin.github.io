...
topic: 13
order: 413
permalink: /teaching-topics/ch4-p13-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch4-p12-candidate/)介紹[伽瑪分配](/teaching-topics/ch4-p12-candidate/#def-gamma-distribution)，把等待第一次偶發事件的[指數分配](/teaching-topics/ch4-p10-candidate/#def-exponential-distribution)推廣為等待第 $\alpha$ 次偶發事件所需要的時間。本篇的韋伯分配是另一個方向的推廣: 偶發事件的次數不變，改為要求 $X$ 除以 $\alpha$ 之後再取 $\gamma$ 次方，所得到的量服從 $\mathrm{Exp}(1)$ 這個分配。

韋伯分配的期望值與變異數都要用[伽瑪函數](/teaching-topics/ch4-p10-candidate/#def-gamma-function)表示，本篇先證明其機率函數為合法的機率函數並求出這兩個量，再說明形狀參數取 $1$ 與取 $2$ 時分別對應到哪一個分配。接著給出可靠度函數與風險函數這兩個定義，它們是描述壽命的兩個基本工具，也正是韋伯分配比指數分配更適合描述壽命的原因所在。最後證明風險函數足以反推出 cdf 與 pdf，並以三道例題作為演練。
...
&-\left[\Gamma\left(\frac{1}{\,\gamma\,}+1\right)\right]^{2}\biggr]
\end{aligned}
$$
...
**Proof.** 先驗證機率函數在值域範圍上的積分為 <span class="text-nowrap">$1$，</span>可得
...
&\quad =\int_{0}^{\infty}\frac{\gamma}{\,\alpha\,}\left(\frac{\,x\,}{\,\alpha\,}\right)^{\gamma-1}e^{-\left(\frac{x}{\alpha}\right)^{\gamma}}\,dx\\[0.45em]
&\quad =\int_{0}^{\infty}e^{-\left(\frac{x}{\alpha}\right)^{\gamma}}\,d\left(\frac{x}{\alpha}\right)^{\gamma}=1
\end{aligned}
$$
...
(4) 韋伯分配相較於[指數分配](/teaching-topics/ch4-p10-candidate/#def-exponential-distribution)而言，更適合用來描述生物或機件的壽命，主要的原因與死亡或損壞的風險有關，見下列關於[可靠度](#def-reliability-function)與[風險](#def-hazard-function)的討論。
{: .topic-paren-item}
...
$$
\begin{gathered}
\mathbb{P}(X>a+b\mid X>a)=\frac{\,\mathbb{P}(X>a+b)\,}{\mathbb{P}(X>a)}=\mathbb{P}(X>b)\\[0.45em]
...
$$
\begin{gathered}
\mathbb{P}(X>a+b\mid X>a)\\[0.3em]
=\frac{\,\mathbb{P}(X>a+b)\,}{\mathbb{P}(X>a)}=\mathbb{P}(X>b)\\[0.55em]
\Longleftrightarrow\ R_{\sssig X}(a+b)=R_{\sssig X}(a)\,R_{\sssig X}(b)
\end{gathered}
$$
...
讀者應該記得，在[韋伯分配的第 (2) 點](#weibull-exp-relation)中我們曾經提過，當 $\gamma=1$ 時，韋伯分配服從指數分配，又當 $\gamma=1$ 時，韋伯分配的風險函數是常數函數，此即稍早所提到的**風險恆定**，亦即風險不隨時間變化。事實上，風險恆定的性質正是[無記憶性](/teaching-topics/ch4-p11-candidate/#thm-memoryless-exp)，此即，只要沒有壞的話，無論現在的時點 (壽命) $t$ 為何，下一個瞬間故障失效的機率密度是固定的，不與 $t$ 有關。
...
$$
\begin{aligned}
&\int_{0}^{x}h_{\sssig X}(t)\,dt\\[0.45em]
&\quad =\int_{0}^{x}-\frac{d}{\,dt\,}\ln\bigl[1-F_{\sssig X}(t)\bigr]\,dt\\[0.45em]
&\quad =-\int_{0}^{x}d\ln\bigl[1-F_{\sssig X}(t)\bigr]\\[0.45em]
&\quad =-\ln\bigl[1-F_{\sssig X}(t)\bigr]^{x}_{0}\\[0.45em]
&\quad =-\ln\bigl[1-F_{\sssig X}(x)\bigr]
\end{aligned}
$$
...
$$
1-F_{\sssig X}(x)=\exp\left\lbrace-\int_{0}^{x}h_{\sssig X}(t)\,dt\right\rbrace\ \Longrightarrow\ F_{\sssig X}(x)=1-\exp\left\lbrace-\int_{0}^{x}h_{\sssig X}(t)\,dt\right\rbrace
$$
...
$$
\begin{gathered}
1-F_{\sssig X}(x)=\exp\left\lbrace-\int_{0}^{x}h_{\sssig X}(t)\,dt\right\rbrace\\[0.45em]
\Longrightarrow\ F_{\sssig X}(x)=1-\exp\left\lbrace-\int_{0}^{x}h_{\sssig X}(t)\,dt\right\rbrace
\end{gathered}
$$
...
<!-- ref-point: 待第三章第 29 篇 (隨機樣本之順序統計量的抽樣分配 cdf，書稿 mathstatch3.tex
     第 5559 行的 Theorem 3.25，anchor 為 #thm-order-stat-samp-dist-cdf) 發布後，將下一句的
     「順序統計量的性質」改為指向該 anchor 的站內連結。 -->
...
&\quad =1-\mathbb{P}(T_{\sssig (1)}>t)\\[0.45em]
&\quad =1-\mathbb{P}\bigl(\min(T_1,T_2,\ldots,T_n)>t\bigr)\\[0.45em]
&\quad =1-\mathbb{P}(T_1>t,T_2>t,\ldots,T_n>t)\\[0.45em]
&\quad =1-\prod_{i=1}^{n}\mathbb{P}(T_i>t)
\end{aligned}
$$
...
$$
\begin{aligned}
&\mathbb{P}(X\geqslant2\mid X\geqslant1)\\[0.45em]
&\quad =\frac{\,\mathbb{P}(X\geqslant2,X\geqslant1)\,}{\mathbb{P}(X\geqslant1)}\\[0.45em]
&\quad =\frac{\,\mathbb{P}(X\geqslant2)\,}{\mathbb{P}(X\geqslant1)}=\frac{\,R_{\sssig X}(2)\,}{R_{\sssig X}(1)}\\[0.45em]
&\quad =\frac{\,e^{-4}\,}{\,e^{-\frac{1}{4}}\,}=e^{-\frac{15}{4}}
\end{aligned}
$$
...
$$
\begin{aligned}
&\mathbb{P}(Y\geqslant60\mid Y\geqslant50)\\[0.45em]
&\quad =\frac{\,\mathbb{P}(Y\geqslant60)\,}{\mathbb{P}(Y\geqslant50)}=0.7
\end{aligned}
$$
...
&\quad =\frac{\,\exp\left\lbrace-\int_{0}^{60}r_{\sssig Y}(t)\,dt\right\rbrace\,}{\,\exp\left\lbrace-\int_{0}^{50}r_{\sssig Y}(t)\,dt\right\rbrace\,}\\[0.45em]
&\quad =\exp\left\lbrace-\int_{50}^{60}r_{\sssig Y}(t)\,dt\right\rbrace=0.7
\end{aligned}
$$
...
&\quad =\frac{\,\exp\left\lbrace-\int_{0}^{60}r_{\sssig X}(t)\,dt\right\rbrace\,}{\,\exp\left\lbrace-\int_{0}^{50}r_{\sssig X}(t)\,dt\right\rbrace\,}\\[0.45em]
&\quad =\exp\left\lbrace-\int_{50}^{60}r_{\sssig X}(t)\,dt\right\rbrace\\[0.45em]
&\quad =\exp\left\lbrace-\int_{50}^{60}2r_{\sssig Y}(t)\,dt\right\rbrace\\[0.45em]
&\quad =\left[\exp\left\lbrace-\int_{50}^{60}r_{\sssig Y}(t)\,dt\right\rbrace\right]^{2}=0.7^{2}
\end{aligned}
$$
...
[Definition 4.15](#def-weibull-distribution) 由 $\left(\frac{\,X\,}{\alpha}\right)^{\gamma}\sim\mathrm{Exp}(1)$ 這條關係界定韋伯分配，其中 $\alpha$ 是比例參數、$\gamma$ 是形狀參數，機率函數為 $\frac{\gamma}{\,\alpha\,}\left(\frac{\,x\,}{\,\alpha\,}\right)^{\gamma-1}e^{-\left(\frac{x}{\alpha}\right)^{\gamma}}$ 這個式子。證明的三個步驟依序是: 把積分變數換成 $\left(\frac{x}{\alpha}\right)^{\gamma}$ 之後直接驗證積分為 <span class="text-nowrap">$1$、</span>由 $Y\sim\mathrm{Exp}(1)$ 的 $k$ 階原動差 $\Gamma(k+1)$ 得到 $X$ 的一二階原動差，再相減得到變異數。期望值與變異數因而都要用[伽瑪函數](/teaching-topics/ch4-p10-candidate/#def-gamma-function)表示。定義之後的四點說明指出: $\gamma=1$ 時韋伯分配即為 <span class="text-nowrap">$\mathrm{Exp}(\beta=\alpha)$、</span>$\gamma=2$ 時被稱作雷利分配並可用來描述兩個獨立同分配的常態變數所構成之隨機向量的長度，以及韋伯分配比[指數分配](/teaching-topics/ch4-p10-candidate/#def-exponential-distribution)更適合描述壽命的原因在於風險。
