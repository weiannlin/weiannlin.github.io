...
topic: 17
order: 317
permalink: /teaching-topics/ch3-p17-candidate/
date: 2026-08-13
published: false
...
[上一篇](/teaching-topics/ch3-p16-candidate/)以[沃德等式](/teaching-topics/ch3-p16-candidate/#thm-wald-identity)處理了加總的項數本身也是隨機變數的情形。本篇轉而處理兩個隨機變數共同變化的情況。

我們在第二章曾以[母體動差](/teaching-topics/ch2-p213-candidate/#def-population-moment)描述單一個機率分配的各種特徵，也曾以[動差母函數](/teaching-topics/ch2-p215-candidate/#def-mgf)協助生成各階的原動差。本篇先把動差推廣為交叉動差，再把動差母函數推廣為聯合動差母函數，接著看聯合動差母函數如何生成各階交叉動差、又如何轉為邊際動差母函數，最後以兩道例題示範它在計算上的用法。
...
若 $X$ 與 $Y$ 為**離散**變數，且對任意正數 <span class="text-nowrap">$h$，</span>下列期望值在 $-h < t_1, t_2 < h$ 皆存在，則
...
&\quad =\mathop{\sum\sum}\limits_{(x,y)\in\mathcal{R}_{\sssig XY}}e^{t_1x+t_2y}p_{\sssig XY}(x,y)
\end{aligned}
$$
...
\begin{aligned}
&M_{\sssig XY}(t_1, t_2)=\mathbb{E}\bigl(e^{t_1X+t_2Y}\bigr)\\[0.45em]
&\quad =\iint_{(x,y)\in\mathcal{R}_{\sssig XY}}e^{t_1x+t_2y}\\[0.2em]
&\qquad\quad f_{\sssig XY}(x,y)\,dx\,dy
\end{aligned}
$$
...
聯合動差母函數 (joint mgf) 與單變數的 mgf 相同，並不是任何時候都存在。一個 joint mgf 必須在 $-h < t_1, t_2 < h$ 的區間內皆有定義才存在，並且一旦 joint mgf 存在，即**保證了該二個隨機變數的各階交叉動差都存在**。
...
$$
\begin{aligned}
&\left.\frac{\partial^{n+m}M_{\sssig XY}(t_1, t_2)}{\partial t_1^{n}\,\partial t_2^{m}}\right|_{t_1=t_2=0}\\[0.45em]
&\quad =\mathbb{E}\bigl(X^nY^m\bigr)
\end{aligned}
$$
...
這個定理的證明是 [Theorem 2.21](/teaching-topics/ch2-p215-candidate/#thm-mgf-generates-moments) 的簡單推廣，故將這個定理的證明省略。有興趣的讀者可自行嘗試推導。

<div class="topic-box topic-box--note" markdown="1">
<div class="topic-box__label">Note</div>

[Theorem 3.13](#thm-generate-cross-moment) 說明了 joint mgf 如何生出兩個隨機變數之間的各階交叉動差。一個簡單的應用是當 $n=m=1$ 時，我們會得到 <span class="text-nowrap">$\mathbb{E}(XY)$，</span>也就是 $X$ 與 $Y$ 的二階交叉動差，這個交叉動差在後續的小節所介紹的[共變異數](/teaching-topics/ch3-p18-candidate/#def-covariance) <span lang="en">(covariance)</span> 會起到關鍵的作用。
...
邊際動差母函數 (marginal mgf) 即為邊際分配的 mgf。
...
&\quad =\mathbb{E}\bigl(e^{t_1X}\bigr)=M_{\sssig X}(t_1)
\end{aligned}\\[0.7em]
\begin{aligned}
&M_{\sssig XY}(0, t_2)=\mathbb{E}\bigl(e^{0\cdot X+t_2Y}\bigr)\\[0.3em]
&\quad =\mathbb{E}\bigl(e^{t_2Y}\bigr)=M_{\sssig Y}(t_2)
\end{aligned}
\end{gathered}
$$
...
&\quad =\int_{-\infty}^{\infty}\!\!\int_{-\infty}^{\infty}e^{t_1x}f_{\sssig XY}(x, y)\,dx\,dy\\[0.45em]
&\quad =\int_{-\infty}^{\infty}e^{t_1x}\biggl[\int_{-\infty}^{\infty}f_{\sssig XY}(x, y)\,dy\biggr]\,dx\\[0.45em]
&\quad =\int_{-\infty}^{\infty}e^{t_1x}f_{\sssig X}(x)\,dx=M_{\sssig X}(t_1)
\end{aligned}
$$
...
&\quad =\left.\frac{\partial}{\partial t_1}\biggl[\frac{\partial M_{\sssig X_1X_2}(t_1, t_2)}{\partial t_2}\biggr]\right|_{t_1=t_2=0}\\[0.45em]
...
&\qquad\qquad\qquad\qquad\qquad\biggr|_{t_1=t_2=0}\\[0.45em]
&\quad =-40\Bigl((1-t_1+2t_2)^{-6}(1-t_1)^{-5}\\[0.25em]
&\qquad\ +(1-t_1+2t_2)^{-5}(1-t_1)^{-6}\Bigr)\biggr|_{t_1=t_2=0}\\[0.45em]
&\quad =-80
\end{aligned}
$$
...
$$
\begin{aligned}
&\Longrightarrow\ \operatorname{Cov}(X_1, X_2)\\[0.45em]
&\quad =\mathbb{E}(X_1X_2)-\mathbb{E}(X_1)\mathbb{E}(X_2)\\[0.45em]
&\quad =-80-9\times(-8)=-8
\end{aligned}
$$
...
[共變異數](/teaching-topics/ch3-p18-candidate/#def-covariance) (covariance) 的定義及算法在稍後即會提到。
...
&\quad =\biggl[\frac{1}{\,3\,}(e^{t_1+t_2}+1)+\frac{1}{\,6\,}(e^{t_1}+e^{t_2})\biggr]^{2}
\end{aligned}
$$
...
\begin{aligned}
&M_{\sssig XY}(t_1, t_2)=\mathbb{E}\Bigl(e^{t_1X+t_2Y}\Bigr)\\[0.45em]
&\quad =\biggl[\frac{1}{\,3\,}(e^{t_1+t_2}+1)+\frac{1}{\,6\,}(e^{t_1}+e^{t_2})\biggr]^{2}\\[0.45em]
&\quad =\frac{1}{\,9\,}e^{2t_1+2t_2}+\frac{1}{\,9\,}e^{2t_1+t_2}\\[0.3em]
...
由[動差母函數的唯一性](/teaching-topics/ch2-p216-candidate/#thm-mgf-uniqueness)可知，$X$ 與 $Y$ 具聯合與邊際機率質量函數如下
...
事實上，joint mgf 與 marginal mgf 一樣，能夠刻畫整個分配的所有特徵，因此也具有如 [Theorem 2.23](/teaching-topics/ch2-p216-candidate/#thm-mgf-uniqueness) 的唯一性定理能夠使用，特別是在離散型隨機向量上，辨認方式完全類同過去，即將 $X, Y$ 的 joint mgf $M_{\sssig XY}(t_1, t_2)$ $=$ $\sum_{(x, y)\in\mathbb{R}^{2}}e^{t_1x+t_2y}\,p_{\sssig XY}(x,y)$ 展開，看成是 $p_{\sssig 1}e^{a_{11}\,t_1+a_{12}\,t_2}$ $+$ $...$ $+$ $p_{\sssig n}e^{a_{n1}\,t_1+a_{n2}\,t_2}$ 的形式，並認出其分配為
...
[Definition 3.14](#def-cross-moment) 把單變數的動差推廣為 $\mathbb{E}(X^nY^m)$ 這種交叉動差，用來描述 $X$ 與 $Y$ 共同變化的各種特徵；[Definition 3.15](#def-joint-mgf) 則把動差母函數推廣為聯合動差母函數，離散型以聯合機率質量函數加權求和、連續型以聯合機率密度函數加權積分。與單變數的 mgf 相同，joint mgf 必須在 $-h < t_1, t_2 < h$ 的整個區間內皆有定義才算存在，而一旦它存在，各階交叉動差也就都跟著存在。

生成的方式由 [Theorem 3.13](#thm-generate-cross-moment) 給出: 對 $t_1$ 微分 $n$ 次、對 $t_2$ 微分 $m$ 次，再代入 <span class="text-nowrap">$t_1=t_2=0$，</span>所得即為 $(n+m)$ 階交叉動差。其中 $n=m=1$ 的情形給出 $\mathbb{E}(XY)$ 這個量，它是下一篇定義共變異數時所需的關鍵一項。[Theorem 3.14](#thm-marginal-mgf) 則指出，只要把 $t_1$ 與 $t_2$ 其中一個代入 <span class="text-nowrap">$0$，</span>該變數在指數上的那一項就整個消去，剩下的正是另一個變數自己的動差母函數；證明只需把聯合機率密度函數對其中一個變數積分，得到的就是邊際機率密度函數。

兩道例題示範它在計算上的用法。[Example 3.35](#ex-joint-mgf-two-variables) 先對 $t_2$ 再對 $t_1$ 微分求得 <span class="text-nowrap">$\mathbb{E}(X_1X_2)=-80$，</span>再由 $M_{\sssig X_1X_2}(t_1, 0)$ 與 $M_{\sssig X_1X_2}(0, t_2)$ 取得兩個邊際動差母函數，各微分一次得到 $9$ 與 <span class="text-nowrap">$-8$，</span>合起來即得共變異數為 <span class="text-nowrap">$-8$。</span>[Example 3.36](#ex-joint-mgf-covariance) 則把聯合動差母函數的平方展開成九項，每一項的係數就是一個機率、指數上的兩個係數就是一組取值，由[動差母函數的唯一性](/teaching-topics/ch2-p216-candidate/#thm-mgf-uniqueness)即可把整張聯合與邊際機率質量函數的表填出來。離散型的辨認方式與單變數時完全相同，差別只在指數上由一個變數變成兩個。

[下一篇](/teaching-topics/ch3-p18-candidate/)正式給出共變異數的定義，並說明它與二階交叉動差 $\mathbb{E}(XY)$ 之間的關係。
