...
topic: 2
order: 302
permalink: /teaching-topics/ch3-p02-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch3-p01-candidate/)由隨機向量的定義出發，介紹了二元離散型隨機向量的[聯合機率質量函數](/teaching-topics/ch3-p01-candidate/#def-joint-pmf)與[邊際機率質量函數](/teaching-topics/ch3-p01-candidate/#def-marginal-pmf)。聯合機率質量函數只定義在離散型的隨機向量上，而第二章的[累積分配函數](/teaching-topics/ch2-p02-candidate/#def-cdf)則是由 $\mathbb{P}(X\leqslant x)$ 出發，離散型與連續型都適用。

二元的情形同樣可以這樣做。[Definition 3.1](/teaching-topics/ch3-p01-candidate/#def-random-vector) 要求對任意實數 $x_1,x_2,\ldots,x_n$ 而言，能使 $X_i(\omega)\leqslant x_i$ 對每一個 $i$ 都成立的那些樣本點所構成的集合是事件，故在 $n=2$ 的情況中，$\mathbb{P}(X\leqslant x,Y\leqslant y)$ 這個機率對平面上每一個點 $(x,y)$ 都有定義。把 $(x,y)$ 看成變數，這個機率本身就是一個定義在二維實數平面上的函數。
...
讀者或許已經發現，上列這些性質，事實上與單變數 cdf 相去不遠，很多部分甚至一樣，則我們是不是能夠用與單變數時相同的手法，從而定義二元連續型隨機變數的**聯合機率密度函數 (joint pdf)** 呢？
...
這些性質與單變數的 cdf 相去不遠，故我們也能用與單變數時相同的手法，定義二元連續型隨機變數的聯合機率密度函數。[下一篇](/teaching-topics/ch3-p03-candidate/)便由聯合機率密度函數的定義開始，並說明它與聯合累積分配函數之間的關係。
