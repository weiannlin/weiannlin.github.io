...
辛普森悖論則說明另一件事。即使每個分組裡的比較方向都相同，把分組混合起來以後，整體比較仍可能反向。這種反向來自混合權重的改變，使我們實際比較的對象也跟著改變。

以下固定令 $(S,\mathcal{F},\mathbb{P})$ 為一個機率空間。令 $C_1,\ldots,C_k$ 為樣本空間的一個分割，也就是這些事件彼此互斥，且合起來沒有遺漏。

## 混合後的機率是加權平均

令 $A$ 表示某個處理方式，令 $Y$ 表示成功事件。假設 $\mathbb{P}(A)>0$，且只針對 $\mathbb{P}(A\cap C_i)>0$ 的分組計算條件成功率。若 $A$ 已經發生，則在分割 $C_1,\ldots,C_k$ 下，由全機率定理可得
...
由此可知，整體成功率須以 $\mathbb{P}(C_i\mid A)$ 當權重做加權平均，不能單純把各組成功率平均。若兩個處理方式 $A$ 與 $B$ 對應到的分組比例不同，則它們的整體成功率其實是在用不同權重混合不同族群。
