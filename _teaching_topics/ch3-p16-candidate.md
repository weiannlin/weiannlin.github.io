...
topic: 16
order: 316
permalink: /teaching-topics/ch3-p16-candidate/
date: 2026-08-13
published: false
...
[上一篇](/teaching-topics/ch3-p15-candidate/)以 [Theorem 3.11](/teaching-topics/ch3-p15-candidate/#thm-var-decom-thm) 把一個隨機變數的變異數拆成組內變異與組間變異兩個部分。[雙重期望值定理](/teaching-topics/ch3-p12-candidate/#thm-double-expectation)與[變異數分解定理](/teaching-topics/ch3-p15-candidate/#thm-var-decom-thm)，都是把條件之下的產物依條件變數的機率加權平均，從而得到邊際的產物；這兩條定理還有一個很典型的衍生應用，就是加總的項數本身也是隨機變數的情形。

本篇即由此出發，先給出沃德等式 <span lang="en">(Wald’s identity)</span>，它要求被加總的各項彼此[獨立](/teaching-topics/ch3-p09-candidate/#def-indep-r-v)且具有相同的分配，而加到第幾項則由另一個非負整數隨機變數決定；接著以雙重期望值定理與變異數分解定理完成它的證明，再以四道例題示範它的用法。前兩道例題處理同一個問題: 一道由沃德等式與[全機率定理](/teaching-topics/ch3-p14-candidate/#thm-law-of-total-prob-r-v)求出每日售出 laptop PC 的期望值、標準差與分配，另一道改以動差母函數確認同一個分配。後兩道則是賭徒破產問題，公平賭局的版本可由沃德等式一步得解，不公平賭局的版本則須逐項相減再以等比級數求和。
...
又由[雙重期望值定理](/teaching-topics/ch3-p12-candidate/#thm-double-expectation)可知
{: .topic-paren-cont}
...
(2) 由[變異數分解定理](/teaching-topics/ch3-p15-candidate/#thm-var-decom-thm)可知
{: .topic-paren-item}
...
&\quad +\mathrm{Var}\bigl[\mathbb{E}(S_{\sssig N}\mid N)\bigr]
\end{aligned}
$$
...
證明過程中使用到 $\mathrm{Var}(S_{\sssig N}\mid N=n)$ $=$ $n\sigma_{\sssig X}^{2}$ 的結果，我們至此還沒有說明過原因，但讀者可以不用心急，稍後介紹到共變異數的環節時，我們將會學到所有所需的細節。

</div>

<!-- ref-point: 待第三章篇 18 的共變異數發布後，將本則 Note 的「共變異數」改為指向該處的站內連結。 -->
...
<ol class="topic-list-paren">
  ...
  <li>Write $X$ for the number of laptop PCs sold in a day at Mova’s store, and determine the probability distribution of <span class="text-nowrap">$X$.</span></li>
</ol>
</div>
...
&\quad +\bigl[\mathbb{E}(Y_i)\bigr]^{2}\,\mathrm{Var}(N)\\[0.45em]
&=\lambda p(1-p)+\lambda p^{2}=\lambda p
\end{aligned}
$$
...
$$
\Longrightarrow\ \operatorname{SD}(X)=\sqrt{\mathrm{Var}(X)}=\sqrt{\lambda p}
$$

<div class="topic-box topic-box--note" markdown="1">
<div class="topic-box__label">Note</div>

$\mathrm{Poi}(\lambda)$ 是卜瓦松分配 <span lang="en">(Poisson distribution)</span>，這種分配我們在之後會談到，期望值與變異數都是 <span class="text-nowrap">$\lambda$，</span>是相當特殊的分配。

</div>

<!-- ref-point: 待第四章的卜瓦松分配主題發布後，將本則 Note 的「卜瓦松分配」改為指向該篇的站內連結。 -->

(2) 由[全機率定理](/teaching-topics/ch3-p14-candidate/#thm-law-of-total-prob-r-v)可知
{: .topic-paren-item}
...
&\quad =\sum_{n=0}^{\infty}p_{\sssig X\mid N}(0\mid n)\,p_{\sssig N}(n)\\[0.45em]
&\quad =\sum_{n=0}^{\infty}\binom{n}{0}p^{0}(1-p)^{n-0}\cdot\frac{\,e^{-\lambda}\lambda^{n}\,}{n!}\\[0.45em]
&\quad =e^{-\lambda}\sum_{n=0}^{\infty}\frac{\bigl[\lambda(1-p)\bigr]^{n}}{n!}\\[0.45em]
&\quad =e^{-\lambda}\cdot e^{\lambda(1-p)}=e^{-\lambda p}
\end{aligned}
$$

</div>

(3) 由[全機率定理](/teaching-topics/ch3-p14-candidate/#thm-law-of-total-prob-r-v)可知
{: .topic-paren-item}
...
&\quad =\sum_{n=x}^{\infty}\binom{n}{x}p^{x}(1-p)^{n-x}\cdot\frac{e^{-\lambda}\lambda^{n}}{n!}\\[0.45em]
&\quad =\sum_{n=x}^{\infty}\frac{n!}{x!(n-x)!}\cdot p^{x}(1-p)^{n-x}\\[0.2em]
&\qquad\quad \cdot\frac{e^{-\lambda}\lambda^{n}}{n!}\\[0.2em]
&\qquad\quad \bigl[\,\text{令}\ t=n-x\ \Longleftrightarrow\ n=t+x\,\bigr]\\[0.45em]
&\quad =\sum_{t=0}^{\infty}\frac{e^{-\lambda}\lambda^{t+x}}{x!\,t!}p^{x}(1-p)^{t}\\[0.45em]
&\quad =\frac{e^{-\lambda}(\lambda p)^{x}}{x!}\sum_{t=0}^{\infty}\frac{\bigl[\lambda(1-p)\bigr]^{t}}{t!}\\[0.45em]
&\quad =\frac{e^{-\lambda}(\lambda p)^{x}}{x!}\cdot e^{\lambda(1-p)}\\[0.45em]
&\quad =\frac{e^{-\lambda p}(\lambda p)^{x}}{x!},\ \ x=0,1,\ldots,\infty
\end{aligned}
$$
...
$$
\Longrightarrow\ X\sim\mathrm{Poi}(\lambda p)
$$

<div class="topic-box topic-box--note" markdown="1">
<div class="topic-box__label">Note</div>

上述的[全機率定理](/teaching-topics/ch3-p14-candidate/#thm-law-of-total-prob-r-v)中，之所以 $n$ 的範圍從 $x$ 開始而非從 $0$ 開始，乃是因為其 conditional pmf 的範圍如下
...
這意味著，雖然是給定了 $N=n$ 且 <span class="text-nowrap">$n\in\lbrace 0\rbrace\cup\mathbb{N}$，</span>但是在固定 $X=x$ 時，$n$ 不可能是一個比 $x$ 還要更小的非負整數，所以在上述的[全機率定理](/teaching-topics/ch3-p14-candidate/#thm-law-of-total-prob-r-v)中，加總的起點是 $x$ 而非 <span class="text-nowrap">$0$。</span>
...
&\quad =\sum_{y=0}^{\infty}\bigl[\,pe^{t}+(1-p)\,\bigr]^{y}\,\frac{\,e^{-\lambda}\lambda^{y}\,}{y!}\\[0.45em]
&\quad =e^{-\lambda}\sum_{y=0}^{\infty}\frac{\bigl(\lambda\,\bigl[\,pe^{t}+(1-p)\,\bigr]\bigr)^{y}}{y!}\\[0.45em]
&\quad =e^{\lambda p(e^{t}-1)},\ t\in\mathbb{R}
\end{aligned}
$$
...
事實上，這題是[上一個問題](#ex-store-laptop-purchases)第 (3) 小題的另一種做法，也就是由 [mgf](/teaching-topics/ch2-p215-candidate/#def-mgf) 來確認 $\sum_{i=1}^{Y}X_i$ 的分配。

儘管我們還沒有正式講到多元隨機變數的 mgf 以及其衍生的計算方式，但是讀者仍然可以把 $e^{t(X_1+X_2+\cdots+X_{\sssig Y})}$ 當成 $X_1, X_2, \ldots, X_{\sssig Y}$ 的函數，並且利用彼此獨立的性質，改為邊際的 mgf 相乘，從而得到所求結果；而這也是[雙重期望值定理](/teaching-topics/ch3-p12-candidate/#thm-double-expectation)的另一個應用，即
...
$$
\begin{aligned}
&\mathbb{E}\bigl[e^{t(X_1+\cdots+X_{\sssig Y})}\bigr]\\[0.45em]
&\quad =\mathbb{E}\Bigl(\mathbb{E}\bigl[e^{t(X_1+\cdots+X_{\sssig Y})}\bigm\vert Y\bigr]\Bigr)
\end{aligned}
$$
...
&\quad +(a+b)\times\mathbb{P}(S_{\sssig T}=a+b)\\[0.45em]
&=(a+b)\times\mathbb{P}(S_{\sssig T}=a+b)
\end{aligned}
$$
...
&\quad +\mathbb{P}(E\mid H^{\prime})\,\mathbb{P}(H^{\prime})\\[0.45em]
&=p_{\sssig i+1}\times p\\[0.2em]
&\quad +p_{\sssig i-1}\times(1-p)
\end{aligned}
$$
...
&\quad =\Bigl(\frac{q}{p}\Bigr)^{\sssig N-1}\,p_{\sssig 1}
\end{aligned}
$$
...
$$
\begin{aligned}
&p_{\sssig i}-p_{\sssig 1}=\frac{q}{p}\,p_{\sssig 1}+\Bigl(\frac{q}{p}\Bigr)^{2}\,p_{\sssig 1}+\cdots+\Bigl(\frac{q}{p}\Bigr)^{\sssig i-1}\,p_{\sssig 1}\\[0.45em]
...
$$
\begin{aligned}
&p_{\sssig i}-p_{\sssig 1}\\[0.2em]
&\quad =\frac{q}{p}\,p_{\sssig 1}+\Bigl(\frac{q}{p}\Bigr)^{2}\,p_{\sssig 1}\\[0.2em]
&\qquad +\cdots+\Bigl(\frac{q}{p}\Bigr)^{\sssig i-1}\,p_{\sssig 1}\\[0.45em]
&\Longrightarrow\ p_{\sssig i}\\[0.2em]
&\quad =p_{\sssig 1}+\frac{q}{p}\,p_{\sssig 1}+\Bigl(\frac{q}{p}\Bigr)^{2}\,p_{\sssig 1}\\[0.2em]
...
\begin{aligned}
&\mathrm{Var}(S_{\sssig T})=\mathbb{E}(S_{\sssig T}^{2})-\bigl[\mathbb{E}(S_{\sssig T})\bigr]^{2}\\[0.45em]
&\quad =\Bigl[0^{2}\,\mathbb{P}(S_{\sssig T}=0)\\[0.2em]
&\qquad +(a+b)^{2}\,\mathbb{P}(S_{\sssig T}=a+b)\Bigr]-a^{2}\\[0.45em]
&\quad =(a+b)^{2}\times\frac{a}{\,a+b\,}-a^{2}\\[0.45em]
&\quad =a\,(a+b)-a^{2}=ab
\end{aligned}
$$
...
&\quad +\bigl[\mathbb{E}(X_i)\bigr]^{2}\,\mathrm{Var}(T)\\[0.45em]
&=\mathrm{Var}(X)\,\mathbb{E}(T)
\end{aligned}
$$
...
$$
\Longrightarrow\ \mathbb{E}(T)=\frac{\,\mathrm{Var}(S_{\sssig T})\,}{\mathrm{Var}(X)}=\frac{ab}{\,\mathrm{Var}(X)\,}
$$
...
然而，儘管我們可以有各種方式取得邊際產物，但很多時候，我們並不會單純地只想看見邊際的產物而已，隨機變數之間**共同變化的情況**也會是我們所關注的重點之一，見下列小節。

<!-- ref-point: 待第三章第 17 篇發布後，將本段的「下列小節」改為指向該篇的站內連結。 -->

## 本篇小結

[Theorem 3.12](#thm-wald-identity) 處理的是加總的項數本身也是隨機變數的情形: 若 $X_1, X_2, \ldots$ 為獨立同分配的隨機變數，$N$ 是一個與它們獨立的非負整數隨機變數，則 $S_{\sssig N}=\sum_{i=1}^{N}X_i$ 的期望值為 <span class="text-nowrap">$\mu_{\sssig X}\mu_{\sssig N}$，</span>變異數則為 <span class="text-nowrap">$\sigma_{\sssig X}^{2}\mu_{\sssig N}+\mu_{\sssig X}^{2}\sigma_{\sssig N}^{2}$。</span>它的證明分別以[雙重期望值定理](/teaching-topics/ch3-p12-candidate/#thm-double-expectation)與[變異數分解定理](/teaching-topics/ch3-p15-candidate/#thm-var-decom-thm)完成，所需的兩個條件量是 $\mathbb{E}(S_{\sssig N}\mid N=n)$ $=$ $n\mu_{\sssig X}$ 與 $\mathrm{Var}(S_{\sssig N}\mid N=n)$ $=$ $n\sigma_{\sssig X}^{2}$ 這兩條式子。

[Example 3.31](#ex-store-laptop-purchases) 是沃德等式最直接的用法: 每日進入商店的人數為卜瓦松分配，每位顧客購買 laptop PC 之數量則為 <span class="text-nowrap">$\mathrm{Ber}(p)$，</span>兩者合起來得到 $\mathbb{E}(X)=\lambda p$ 與 <span class="text-nowrap">$\operatorname{SD}(X)=\sqrt{\lambda p}$；</span>再以[全機率定理](/teaching-topics/ch3-p14-candidate/#thm-law-of-total-prob-r-v)把條件 pmf 依 $N$ 的機率加權，可得 $\mathbb{P}(X=0)$ $=$ $e^{-\lambda p}$ 以及 <span class="text-nowrap">$X\sim\mathrm{Poi}(\lambda p)$。</span>這裡的加總起點之所以是 $x$ 而非 <span class="text-nowrap">$0$，</span>是因為在固定 $X=x$ 之後，$n$ 不可能小於 <span class="text-nowrap">$x$。</span>[Example 3.32](#ex-poisson-random-sum) 改用另一種做法: 先求出給定 $Y=y$ 之下的 $\phi(t,y)$ $=$ $\bigl[\,pe^{t}+(1-p)\,\bigr]^{y}$ 這條式子，再對 $Y$ 取一次期望值，得到 <span class="text-nowrap">$e^{\lambda p(e^{t}-1)}$，</span>正是同一個分配的 mgf。
...
至此，所有「給定條件下」的情形都已看過。下一篇轉而處理兩個隨機變數共同變化的情況，由交叉動差與聯合動差母函數談起。

<!-- ref-point: 待第三章第 17 篇發布後，將上一段的「下一篇」改為站內連結。 -->
