...
因此，$[1,4]$ 中的每一個點都可作為 $1/2$ 分位數。注意 $2$ 與 $3$ 並不是 $X$ 的可能取值，仍然可以是這個意義下的分位數。分位數描述的是把機率分在左右兩側的位置，不必然要求該位置本身是隨機變數實際可能取到的值。若採用下方的分位函數作為代表值，則此例會選到左端點 $x_{0.5}=1$。
</div>
