...
topic: 5
order: 205
permalink: /teaching-topics/ch2-p05-candidate/
date: 2026-08-04
published: false
...
到目前為止，我們把隨機變數分為離散型與連續型兩類: 離散型以[機率質量函數](/teaching-topics/ch2-p01-candidate/)記錄每一個質點的單點機率，其 cdf 是在質點上跳躍的階梯函數；連續型以[機率密度函數](/teaching-topics/ch2-p03-candidate/)描述機率的疏密，[任一單點的機率皆為 $0$](/teaching-topics/ch2-p04-candidate/#thm-point-probability-zero)，其 cdf 因而處處連續。
...
\begin{array}{@{}l@{}}
0.2,\ \ x=0\\[0.55em]
\dfrac{d\,(1-0.8e^{-x})}{dx}\\[0.2em]
\quad =0.8\,e^{-x},\ \ x>0\\[0.55em]
0,\ \ \text{otherwise}
\end{array}
\right.
$$
...
$$
F_{\sssig X}(x)=\alpha\,F_{\sssig d}(x)+(1-\alpha)\,F_{\sssig c}(x),\quad 0\leqslant\alpha\leqslant1
$$
...
$$
\begin{gathered}
F_{\sssig X}(x)=\alpha\,F_{\sssig d}(x)+(1-\alpha)\,F_{\sssig c}(x)\\[0.5em]
0\leqslant\alpha\leqslant1
\end{gathered}
$$
...
讀者應該沒有忘記，[Theorem 2.1](/teaching-topics/ch2-p02-candidate/#thm-cdf-properties) (cdf 的性質) 中指出，任何的 cdf (不論離散或連續)，其函數值都是從 $0$ 開始，而以 $1$ 結尾，因此需要進行修正，才能夠將其變為正規的 cdf。
...
[下一節](/teaching-topics/ch2-p06-candidate/)開始討論隨機變數的期望值、變異數、標準差與其他量數，先由其中的期望值談起。
