...
topic: 2
order: 402
permalink: /teaching-topics/ch4-p02-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch4-p01-candidate/)由[伯努利實驗](/teaching-topics/ch4-p01-candidate/#def-ber-trial)出發，給出[伯努利分配](/teaching-topics/ch4-p01-candidate/#def-bernoulli)的定義，並在最後留下一個問題: 只有成功與失敗兩種結果的情境下，為什麼要刻意把 $X$ 定義成一次伯努利實驗中的「成功次數」。本篇的二項分配就是這個問題的答案。伯努利實驗的定義本來就包含「每次實驗的成功機率固定」與「實驗與實驗之間彼此獨立」這兩點，因此把同一個伯努利實驗連續進行 $n$ 次是很自然的做法，而此時值得記錄的量，就是這 $n$ 次之中成功的總次數。
...
&\qquad\qquad \times p^{x-1}(1-p)^{n-x}\\[0.45em]
...
&\quad =\sum_{x=0}^{n}x(x-1)\binom{n}{x}p^{x}(1-p)^{n-x}\\[0.45em]
&\quad =\sum_{x=0}^{n}x(x-1)\frac{n!}{x!(n-x)!}p^{x}(1-p)^{n-x}\\[0.45em]
&\quad =n(n-1)p^{2}\sum_{x=2}^{n}\frac{(n-2)!}{(x-2)!(n-x)!}\\[0.25em]
&\qquad\qquad \times p^{x-2}(1-p)^{n-x}\\[0.45em]
&\quad =n(n-1)p^{2}\bigl[p+(1-p)\bigr]^{(x-2)+(n-x)}\\[0.25em]
&\quad =n(n-1)p^{2}
\end{aligned}
$$
...
在證明的過程中，我們使用到了 [Theorem 2.18](/teaching-topics/ch2-p213-candidate/#thm-binomial) 中提到的二項式定理 <span lang="en">(binomial theorem)</span>。
...
<!-- ref-point: 待第三章第 25 篇 (獨立隨機變數的線性組合之動差母函數，書稿 mathstatch3.tex
     ...
     本篇另有一處同樣待回填，見 Example 4.7 第 (2) 小題。 -->
...
&\quad =\bigl[pe^{t}+(1-p)\bigr]^{n_1}\bigl[pe^{t}+(1-p)\bigr]^{n_2}\\[0.45em]
&\quad =\bigl[pe^{t}+(1-p)\bigr]^{n_1+n_2}
\end{aligned}
$$

</div>

則由 [mgf 的唯一性](/teaching-topics/ch2-p216-candidate/#thm-mgf-uniqueness)可知
...
<!-- fig-pending: binomial-pmf-shapes
     Fig. 4.1，三個面板，對應書稿 mathstatch4.tex 第 211、238、265 行的三個 tikzpicture
     (三者各包在一個 .33\textwidth 的 minipage 內，書稿橫排成一列; 網頁在 390 px 應改為
     縱向堆疊或維持三欄但各自縮小，實際擇一時以 350 px 的可讀性為準)。
     三個面板畫的都是 n = 10 的二項機率函數，以 pgfplots 的 only marks 畫散點 (不畫線)，
     取樣點為 x = 0, 1, ..., 10，顏色 black，scale = 0.7。
     三個面板共同的座標設定: 只畫 x 軸 (axis x line=center)，y 軸不畫 (axis y line=none)，
     y 軸不顯示刻度數值，兩軸皆不寫軸名，xmin = -0.5、xmax = 11、ymin = -0.01。
     各面板的差異只有成功機率與 y 軸上界:
       左面板 (書稿第 211 行): Bin(10, 0.2)，ymax = 0.4，圖例文字 $\mathrm{Bin}(10,0.2)$。
                                對應正文的「右偏」(p 小於 0.5)。
       中面板 (書稿第 238 行): Bin(10, 0.5)，ymax = 0.3，圖例文字 $\mathrm{Bin}(10,0.5)$。
                                對應正文的「對稱」(p 等於 0.5)。
       右面板 (書稿第 265 行): Bin(10, 0.8)，ymax = 0.4，圖例文字 $\mathrm{Bin}(10,0.8)$。
                                對應正文的「左偏」(p 大於 0.5)。
     書稿的 \addlegendentry 在網頁改為各面板下方的一行標示即可，圖例框不必保留。
     三個面板的 y 軸上界不同 (0.4、0.3、0.4)，若要讓讀者直接比較高度，畫圖時須決定是照
     書稿保留不同上界，還是統一為 0.4; 照書稿保留為預設，變更須先報作者裁定。
     檔名 binomial-pmf-shapes.svg，anchor 取 #fig-binomial-pmf-shapes，
     caption 起首為 Fig. 4.1.，內容說明 p 小於、等於、大於 0.5 時分別對應右偏、對稱與左偏。
     圖畫好之後，上一段末的「產生改變」之後補上指向該 anchor 的 Fig. 4.1 連結。 -->
...
(1) 由 [mgf 的唯一性](/teaching-topics/ch2-p216-candidate/#thm-mgf-uniqueness)可知
{: .topic-paren-item}

<div class="topic-math-layout topic-math-layout--desktop" markdown="1">

$$
X\sim\mathrm{Bin}(n=16,\ p=0.75)\ \Longrightarrow\ p_{\sssig X}(x)=\binom{16}{x}(0.75)^{x}(0.25)^{16-x},\ x=0,1,\ldots,16
$$
...
$$
\begin{aligned}
&X\sim\mathrm{Bin}(n=16,\ p=0.75)\\[0.45em]
&\quad \Longrightarrow\ p_{\sssig X}(x)=\binom{16}{x}(0.75)^{x}(0.25)^{16-x},\\[0.25em]
...
&\quad +\binom{8}{8}(0.6)^{8}(0.4)^{0}\\[0.45em]
&\fallingdotseq0.1064
\end{aligned}
$$
...
<!-- ref-point: 待第三章第 12 篇 (雙重期望值定理，書稿 mathstatch3.tex 第 2267 行的
     Theorem 3.9，anchor 為 #thm-double-expectation) 發布後，將下一句的「雙重期望值定理」
     改為指向該 anchor 的站內連結。 -->
...
&\quad +100\times0.25\times\frac{1}{\,2\,}\\[0.45em]
&=37.5
\end{aligned}
$$
...
依題意可以知道 $N\sim\mathrm{Bin}(m,\ p)$ 及 <span class="text-nowrap">$(Y\mid N=n)\sim\mathrm{Bin}(n,\ q)$，</span>則由全機率定理可知

<!-- ref-point: 待第三章第 14 篇 (全機率定理，書稿 mathstatch3.tex 第 2512 行的
     Theorem 3.10，anchor 為 #thm-law-of-total-prob-r-v) 發布後，將上一句的「全機率定理」
     改為指向該 anchor 的站內連結。 -->
...
&\quad =\sum_{n=y}^{m}\binom{n}{y}q^{y}(1-q)^{n-y}\\[0.25em]
&\qquad\qquad \times\binom{m}{n}p^{n}(1-p)^{m-n}\\[0.45em]
&\quad =\sum_{n=y}^{m}\frac{n!}{\,y!(n-y)!\,}q^{y}(1-q)^{n-y}\\[0.25em]
&\qquad\qquad \times\frac{m!}{\,n!(m-n)!\,}p^{n}(1-p)^{m-n}\\[0.45em]
&\quad =\frac{m!}{\,(m-y)!\,y!\,}q^{y}(1-q)^{-y}\\[0.25em]
&\qquad \times\sum_{n=y}^{m}\frac{(m-y)!}{\,(n-y)!(m-n)!\,}\\[0.25em]
&\qquad\qquad \times\bigl[p(1-q)\bigr]^{n}(1-p)^{m-n}\\[0.45em]
&\quad =\frac{m!}{\,(m-y)!\,y!\,}q^{y}(1-q)^{-y}\bigl[p(1-q)\bigr]^{y}\\[0.25em]
&\qquad \times\sum_{n=y}^{m}\frac{(m-y)!}{\,(n-y)!(m-n)!\,}\\[0.25em]
&\qquad\qquad \times\bigl[p(1-q)\bigr]^{n-y}(1-p)^{m-n}\\[0.45em]
&\quad =\frac{m!}{\,(m-y)!\,y!\,}(pq)^{y}\\[0.25em]
&\qquad\qquad \times\bigl[p(1-q)+(1-p)\bigr]^{m-y}\\[0.45em]
&\quad =\binom{m}{y}(pq)^{y}(1-pq)^{m-y},\\[0.25em]
...
(1) 依 [mgf 的唯一性](/teaching-topics/ch2-p216-candidate/#thm-mgf-uniqueness)可知 <span class="text-nowrap">$X_1\sim\mathrm{Bin}\bigl(4,\ \frac{2}{\,3\,}\bigr)$，</span>又 $X_2$ 的機率函數為
{: .topic-paren-item}
...
$$
\begin{aligned}
&\mathbb{P}(X_1+X_2=4)\\[0.45em]
&\quad =p_{\sssig X_1X_2}(1,3)+p_{\sssig X_1X_2}(2,2)\\[0.25em]
&\qquad\qquad +p_{\sssig X_1X_2}(3,1)\\[0.45em]
&\quad =p_{\sssig X_1}(1)\,p_{\sssig X_2}(3)+p_{\sssig X_1}(2)\,p_{\sssig X_2}(2)\\[0.25em]
&\qquad\qquad +p_{\sssig X_1}(3)\,p_{\sssig X_2}(1)\\[0.45em]
&\quad =\binom{4}{1}\Bigl(\frac{2}{\,3\,}\Bigr)^{1}\Bigl(\frac{1}{\,3\,}\Bigr)^{3}\times\frac{2}{\,5\,}\\[0.25em]
...
&\quad \times\Bigl(\frac{2}{\,5\,}e^{-3t}+\frac{1}{\,5\,}e^{-6t}+\frac{2}{\,5\,}e^{-9t}\Bigr)
\end{aligned}
$$
...
[Definition 4.3](#def-binomial) 把伯努利分配推廣到 $n$ 次實驗: $X$ 記錄 $n$ 次伯努利實驗中成功的次數，值域為 <span class="text-nowrap">$\lbrace\,0,1,\ldots,n\,\rbrace$，</span>機率函數為 <span class="text-nowrap">$\binom{n}{x}p^{x}q^{n-x}$，</span>其中的組合數對應「哪幾次成功」的選法，而 $n$ 是一個固定的常數，不是參數。證明的四個步驟依序驗證機率函數的加總為 <span class="text-nowrap">$1$、</span>求得 <span class="text-nowrap">$\mathbb{E}(X)=np$、</span>再以階乘動差 $\mathbb{E}\bigl[X(X-1)\bigr]=n(n-1)p^{2}$ 得到 $\mathbb{E}\bigl(X^{2}\bigr)$ 進而算出 <span class="text-nowrap">$\mathrm{Var}(X)=npq$，</span>最後直接由定義求得 <span class="text-nowrap">$M_{\sssig X}(t)=\bigl(pe^{t}+q\bigr)^{n}$。</span>其中機率函數的加總、兩個動差的計算與動差母函數的推導，都用到 [Theorem 2.18](/teaching-topics/ch2-p213-candidate/#thm-binomial) 的二項式定理。

定義之後的幾點說明依序是: $n=1$ 時二項分配即為伯努利分配、成功機率相同且彼此獨立的兩個二項變數相加仍為二項分配 (可加性)、機率函數中的組合數所對應的直觀意義，以及 $p$ 的大小決定分配的形狀，$p$ 大於、等於、小於 $0.5$ 時分別為左偏、對稱與右偏。可加性的證明只需把兩個 mgf 相乘，指數因而相加，再由 [mgf 的唯一性](/teaching-topics/ch2-p216-candidate/#thm-mgf-uniqueness)辨識出結果；同一個論證也給出 $n$ 個獨立同分配的伯努利變數相加即為二項分配。
...
[下一篇](/teaching-topics/ch4-p03-candidate/)把伯努利實驗的兩個類別推廣為 $k$ 個互斥類別，先給出三項式定理，再定義多項實驗與多項分配。
