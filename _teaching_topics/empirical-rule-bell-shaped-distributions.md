...
上列第 (2) 點所說的「從常態分配而來」，具體的算法如下。$X$ 確實服從常態分配時，令 $Z=\frac{\,X-\mu_{\sssig X}\,}{\sigma_{\sssig X}}$，則 $Z$ 服從**標準常態分配 <span lang="en">(standard normal distribution)</span>**；以 $\Phi$ 表示 $Z$ 的 [cdf](/teaching-topics/cumulative-distribution-functions/#def-cdf)，則對任意 $k>0$ 皆有
