...
本篇便由這個對稱性出發，先以 [Theorem 3.24](#thm-order-stat-samp-dist-pdf) 列出隨機樣本之順序統計量的五個 pdf 並證明其中的前兩個，接著以四張圖改由排列組合的觀點重新理解這五個結果，最後以標準均勻分配為例，說明它的順序統計量為什麼會是貝塔分配。
...
&\quad \times\bigl[F_{\sssig X}(y_i)\bigr]^{i-1}\bigl[F_{\sssig X}(y_j)-F_{\sssig X}(y_i)\bigr]^{j-i-1}\\[0.45em]
&\quad \times\bigl[1-F_{\sssig X}(y_j)\bigr]^{n-j}, \ y_i, y_j\in \mathcal{R}_{\sssig X} \ \text{且} \ y_i<y_j
\end{aligned}
$$
  </div>
  <div class="topic-math-layout topic-math-layout--mobile">
$$
\begin{aligned}
&f_{\sssig Y_iY_j}(y_i, y_j)\\[0.45em]
&\quad = \frac{n!}{(i-1)!(j-i-1)!(n-j)!}\\[0.45em]
...
<!-- fig-pending: order-stat-min
     Fig. 3.24，對應書稿 mathstatch3.tex 第 5272 至 5286 行的 tikzpicture，單面板。
     內容: 最小順序統計量 $Y_1$ 的示意圖，畫的是一條水平數線與其上的一個切點。
     一條帶箭頭的水平線自 (0, 0) 畫到 (10, 0) (書稿用 ->、>=stealth)，即整條數線。
     切點在 x=3 處: 一道自 (3, 0.5) 垂直畫到 (3, -0.1) 的短線，短線下方標 $y_1$。
     數線上方兩處文字 (書稿放在 y=0.8 的下方): x=1.5 處標「$0$ 個」，x=6.5 處標
     「$n-1$ 個」，分別表示落在 $y_1$ 左側與右側的樣本個數。
     數線下方兩條虛線弧 (書稿用 \path[dashed] ... edge[below, bend right]):
     第一條自 (0, 0) 彎到 (3, 0)，弧下標 $F_{\sssig X}(y_1)$；
     第二條自 (3, 0) 彎到 (9.95, 0) (bend right=14)，弧下標 $1-F_{\sssig X}(y_1)$。
     兩條弧所標的就是正文所說的「一個樣本落在其左側」與「落在其右側」的機率。
     配色與線寬依 CH3_FIGURE_SPECS.md 第一節: 主線與箭頭用 journalink、0.8pt，
     虛線弧用 0.5pt。本圖與 Fig. 3.25、3.26、3.27 為同一系列，四張的數線長度、
     切點高度與字級必須一致，否則並讀時會顯得大小不一。
     檔名 order-stat-min.svg，anchor 取 #fig-order-stat-min。
     圖畫好之後，上一段末的「下面幾張圖」與下一段開頭的「上述這張圖」
     一併改為指向該 anchor 的 Fig. 3.24 連結，並補上 caption。
-->
...
<!-- fig-pending: order-stat-max
     Fig. 3.25，對應書稿 mathstatch3.tex 第 5297 至 5311 行的 tikzpicture，單面板。
     內容: 最大順序統計量 $Y_n$ 的示意圖，版式與 Fig. 3.24 完全相同，只有切點位置、
     兩側個數與兩段機率的標示不同。
     一條帶箭頭的水平線自 (0, 0) 畫到 (10, 0)。
     切點在 x=7 處: 一道自 (7, 0.5) 垂直畫到 (7, -0.1) 的短線，短線下方標 $y_n$。
     數線上方兩處文字 (書稿放在 y=0.8 的下方): x=3.5 處標「$n-1$ 個」，x=8.5 處標
     「$0$ 個」，即左側有 $n-1$ 個樣本、右側沒有樣本。
     數線下方兩條虛線弧: 第一條自 (0, 0) 彎到 (7, 0) (bend right=14)，弧下標
     $F_{\sssig X}(y_n)$；第二條自 (7, 0) 彎到 (9.95, 0)，弧下標 $1-F_{\sssig X}(y_n)$。
     配色與線寬依 CH3_FIGURE_SPECS.md 第一節，與 Fig. 3.24 一致。
     檔名 order-stat-max.svg，anchor 取 #fig-order-stat-max。
     圖畫好之後，上一段末的「示意圖如下」改為指向該 anchor 的 Fig. 3.25 連結，
     並補上 caption。
-->
...
<!-- fig-pending: order-stat-ith
     Fig. 3.26，對應書稿 mathstatch3.tex 第 5324 至 5338 行的 tikzpicture，單面板。
     內容: 一般的第 $i$ 個順序統計量 $Y_i$ 的示意圖，版式與 Fig. 3.24、3.25 相同。
     一條帶箭頭的水平線自 (0, 0) 畫到 (10, 0)。
     切點在 x=5 處: 一道自 (5, 0.5) 垂直畫到 (5, -0.1) 的短線，短線下方標 $y_i$。
     數線上方兩處文字 (書稿放在 y=0.8 的下方): x=2.5 處標「$i-1$ 個」，x=7.5 處標
     「$n-i$ 個」，即左側有 $i-1$ 個樣本、右側有 $n-i$ 個樣本。
     數線下方兩條虛線弧: 第一條自 (0, 0) 彎到 (5, 0)，弧下標 $F_{\sssig X}(y_i)$；
     第二條自 (5, 0) 彎到 (9.95, 0)，弧下標 $1-F_{\sssig X}(y_i)$。
     兩條弧都用書稿預設的 bend right (不是 Fig. 3.24、3.25 用到的 bend right=14)。
     配色與線寬依 CH3_FIGURE_SPECS.md 第一節，與 Fig. 3.24、3.25 一致。
     檔名 order-stat-ith.svg，anchor 取 #fig-order-stat-ith。
     圖畫好之後，上一段末的「如下圖」改為指向該 anchor 的 Fig. 3.26 連結，
     並補上 caption。
-->
...
<!-- fig-pending: order-stat-ij
     Fig. 3.27，對應書稿 mathstatch3.tex 第 5354 至 5373 行的 tikzpicture，單面板。
     內容: 兩個順序統計量 $Y_i$ 與 $Y_j$ 的聯合情形，版式與 Fig. 3.24 至 3.26 相同，
     但數線上有兩個切點、三段區間。
     一條帶箭頭的水平線自 (0, 0) 畫到 (10, 0)。
     兩個切點: 一道自 (3, 0.5) 垂直畫到 (3, -0.1) 的短線，短線下方標 $y_i$；
     另一道自 (7, 0.5) 垂直畫到 (7, -0.1) 的短線，短線下方標 $y_j$。
     數線上方三處文字 (書稿放在 y=0.8 的下方): x=1.5 處標「$i-1$ 個」、x=5 處標
     「$j-i-1$ 個」、x=8.5 處標「$n-j$ 個」，即三段各有 $i-1$、$j-i-1$、$n-j$ 個樣本。
     數線下方三條虛線弧: 第一條自 (0, 0) 彎到 (3, 0)，弧下標 $F_{\sssig X}(y_i)$；
     第二條自 (3, 0) 彎到 (7, 0) (bend right=15)，弧下標
     $F_{\sssig X}(y_j) - F_{\sssig X}(y_i)$；第三條自 (7, 0) 彎到 (9.95, 0)，弧下標
     $1-F_{\sssig X}(y_j)$。中間那條弧的標示較長，要注意不要壓到兩側的弧與切點標示。
     配色與線寬依 CH3_FIGURE_SPECS.md 第一節，與 Fig. 3.24 至 3.26 一致。
     檔名 order-stat-ij.svg，anchor 取 #fig-order-stat-ij。
     圖畫好之後，上一段末的「下圖」改為指向該 anchor 的 Fig. 3.27 連結，
     並補上 caption。
-->
...
<!-- ref-point: 待第四章的均勻分配與貝塔分配主題發布後，將本節首次出現的
     $\mathcal{U}(0,1)$ 與 Beta 兩個分配名稱改為指向該處的站內連結。 -->
...
<!-- fig-pending: order-stat-presorted
     Fig. 3.28，對應書稿 mathstatch3.tex 第 5422 至 5446 行的 tikzpicture，單面板。
     內容: 一條貝塔分配的機率密度曲線，用來說明順序統計量在觀察到值之前就已排好序，
     因此機率密度會集中在它的排序所在的位置。
     以 pgfplots 的 axis 畫，書稿設定為 samples=600、width=.8\textwidth、
     height=.55\textwidth、xmin=0、xmax=1.1、ymin=0、ymax=3.5。
     只畫橫軸 (axis x line=bottom)，縱軸完全不畫 (axis y line=none)，
     縱軸刻度標示為空 (yticklabels=\empty)；橫軸刻度為 xtick={0, 0.1, ..., 1.1}，
     即自 0 至 1.1 每 0.1 一格。兩軸都沒有 xlabel 與 ylabel。
     曲線: 書稿自訂函數 eee(\k,\a,\b)=(\a+\b-1)!/((\a-1)!*(\b-1)!)*\k^(\a-1)*(1-\k)^(\b-1)，
     實際畫的是 \addplot[domain=0:1, black, smooth] {eee(x, 3, 8)}，也就是 Beta(3, 8) 的
     機率密度 360*x^2*(1-x)^7，峰值在 x=2/9、高度約 3.06 (書稿的 ymax=3.5 正是為此而設)，
     右尾長而左尾短。
     網頁改用 journalink、0.8pt 畫這條曲線 (書稿的 black)。
     〔待作者裁定，見本篇回報〕書稿第 5444 行的 \addlegendentry 寫的是
     $\m{Beta}(a=3, b=7)$，與實際代入的 eee(x, 3, 8) 不一致 (畫的是 Beta(3, 8))。
     圖說與圖例究竟取哪一組參數，等作者裁定之後再繪；在此之前不要自行選一組。
     檔名 order-stat-presorted.svg，anchor 取 #fig-order-stat-presorted。
     圖畫好之後，上一段末的「下圖」與下一段的「貝塔分配的圖形」
     一併改為指向該 anchor 的 Fig. 3.28 連結，並補上 caption。
-->
