...
topic: 9
order: 409
permalink: /teaching-topics/ch4-p09-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch4-p08-candidate/)給出[卜瓦松過程](/teaching-topics/ch4-p08-candidate/#def-poisson-process)與[卜瓦松分配](/teaching-topics/ch4-p08-candidate/#def-poisson-distribution)的定義，並完整推導了卜瓦松分配的機率函數、期望值、變異數與動差母函數。本篇先以兩道例題演練卜瓦松機率的計算，其中一道用到二項機率的卜瓦松近似。
...
\begin{gathered}
X\mid(X+Y=n)\\[0.4em]
\sim\mathrm{Bin}\biggl(n,\ p=\frac{\lambda_1}{\,\lambda_1+\lambda_2\,}\biggr)
\end{gathered}
$$
...
$$
\begin{aligned}
&p_{\sssig X\mid X+Y}(x\mid n)\\[0.35em]
&\quad =\frac{\,p_{\sssig XY}(x=x,\ y=n-x)\,}{p_{\sssig X+Y}(n)}\\[0.35em]
&\quad =\frac{\,p_{\sssig X}(x)\,p_{\sssig Y}(n-x)\,}{p_{\sssig W}(n)}\\[0.35em]
&\quad =\frac{\,\frac{\,e^{-\lambda_1}\lambda_1^{x}\,}{x!}\frac{\,e^{-\lambda_2}\lambda_2^{n-x}\,}{(n-x)!}\,}{\frac{\,e^{-(\lambda_1+\lambda_2)}(\lambda_1+\lambda_2)^{n}\,}{n!}}\\[0.35em]
&\quad =\frac{n!}{\,x!(n-x)!\,}\frac{\,\lambda_1^{x}\lambda_2^{n-x}\,}{\,(\lambda_1+\lambda_2)^{n}\,}\\[0.35em]
&\quad =\binom{n}{x}\biggl(\frac{\lambda_1}{\,\lambda_1+\lambda_2\,}\biggr)^{x}\\[0.25em]
&\qquad \cdot\biggl(\frac{\lambda_2}{\,\lambda_1+\lambda_2\,}\biggr)^{n-x},\\[0.25em]
...
$$
\begin{aligned}
&f(x\mid x+y=50)\\[0.35em]
&\quad =\frac{\,f(x=x,\ y=50-x)\,}{f(x+y=50)}\\[0.35em]
&\quad =\frac{\,f_{\sssig X}(x)\,f_{\sssig Y}(50-x)\,}{f_{\sssig W}(50)}\\[0.35em]
&\quad =\frac{\,\frac{\,e^{-m}m^{x}\,}{x!}\frac{\,e^{-n}n^{50-x}\,}{(50-x)!}\,}{\frac{\,e^{-(n+m)}(n+m)^{50}\,}{50!}}\\[0.35em]
&\quad =\frac{50!}{\,x!(50-x)!\,}\frac{\,m^{x}n^{50-x}\,}{\,(n+m)^{50}\,}\\[0.35em]
&\quad =\binom{50}{x}\biggl(\frac{m}{\,n+m\,}\biggr)^{x}\\[0.25em]
&\qquad \cdot\biggl(\frac{n}{\,n+m\,}\biggr)^{50-x},\\[0.25em]
&\qquad x=0,1,2,\ldots,50
\end{aligned}
$$
...
\begin{gathered}
X\mid(X+Y=50)\\[0.4em]
\sim\mathrm{Bin}\biggl(n=50,\ p=\frac{m}{\,m+n\,}\biggr)
\end{gathered}
$$
...
&\quad =\frac{\,e^{-\lambda}\lambda^{x}\,}{x!}\frac{x!}{\,y!(x-y)!\,}p^{y}(1-p)^{x-y}\\[0.35em]
&\quad =\frac{e^{-\lambda}}{\,y!(x-y)!\,}\lambda^{x}p^{y}(1-p)^{x-y},\\[0.25em]
&\qquad y=0,1,2,\ldots,\ x=y,y+1,\ldots
\end{aligned}
$$
...
&\quad =\frac{e^{-\lambda}}{\,u!v!\,}\lambda^{u+v}p^{u}(1-p)^{v}\\[0.35em]
&\quad =\frac{e^{-\lambda}}{\,u!v!\,}\bigl[\lambda(1-p)\bigr]^{v}(\lambda p)^{u},\\[0.25em]
&\qquad u=0,1,2,\ldots,\ v=0,1,2,\ldots\\[0.35em]
&\quad =\frac{\,e^{-\lambda p}(\lambda p)^{u}\,}{u!}\frac{\,e^{-\lambda(1-p)}\bigl[\lambda(1-p)\bigr]^{v}\,}{v!},\\[0.25em]
&\qquad u=0,1,2,\ldots,\ v=0,1,2,\ldots
\end{aligned}
$$
...
\begin{aligned}
&p_{\sssig U}(u)\\[0.35em]
&\quad =\sum_{v=0}^{\infty}\frac{\,e^{-\lambda p}(\lambda p)^{u}\,}{u!}\frac{\,e^{-\lambda(1-p)}\bigl[\lambda(1-p)\bigr]^{v}\,}{v!}\\[0.35em]
&\quad =\frac{\,e^{-\lambda p}(\lambda p)^{u}\,}{u!},\ u=0,1,2,\ldots
\end{aligned}
$$
...
\begin{aligned}
&p_{\sssig V}(v)\\[0.35em]
&\quad =\sum_{u=0}^{\infty}\frac{\,e^{-\lambda p}(\lambda p)^{u}\,}{u!}\frac{\,e^{-\lambda(1-p)}\bigl[\lambda(1-p)\bigr]^{v}\,}{v!}\\[0.35em]
&\quad =\frac{\,e^{-\lambda(1-p)}\bigl[\lambda(1-p)\bigr]^{v}\,}{v!},\ v=0,1,2,\ldots
\end{aligned}
$$
...
上述的兩個問題，是應用在非時間區段的例子，我們曾在[上一篇](/teaching-topics/ch4-p08-candidate/)中提到，卜瓦松過程可以推廣至任意的區間，包含任意線段、平面、空間等等的區間，這當然導致了其上的卜瓦松分配被定義在非時間區段上，但使用的邏輯與定義在時間區段上的卜瓦松分配是完全相同的。
...
[下一篇](/teaching-topics/ch4-p10-candidate/)由伽瑪函數談起，接著給出卜瓦松過程下轄的第二個分配，也就是描述偶發事件發生間隔的指數分配。
