...
本篇是測度觀點的敲門磚。若暫時只需要在有限或可數樣本空間中進行機率計算，可以先掌握「$\mathcal{F}$ 是可被賦予機率的事件清單」這個直覺；若要理解連續機率模型，則需要進一步理解為什麼這份清單必須滿足 $\sigma$-域的封閉性。
</div>

## 從事件到事件集合族

若 $S$ 是樣本空間，則 $S$ 的任一子集合 $A\subset S$ 都可以被直覺地稱為事件。但在建立機率模型時，還需要先指定哪些事件要被納入模型，並且允許機率函數作用在其上。
...
事件集合族不是任意列一串事件就好。若某一事件 $A$ 是我們願意討論的事件，那麼「$A$ 沒有發生」也應該是我們願意討論的事件；若 $A$ 與 $B$ 都是我們願意討論的事件，那麼「$A$ 或 $B$ 發生」以及「$A$ 且 $B$ 發生」也應該是我們願意討論的事件。
...
<ol class="topic-list-paren">
  <li>虛無事件 $\varnothing$ 被納入清單；取其補集可知樣本空間 $S$ 也在清單中。</li>
...
域的直覺如下。只要一個事件被放入清單，它自然衍生出的基本事件也要被放入清單。若 $A$ 可以被討論，則 $A^{\prime}$ 也應該可以被討論；若 $A$ 與 $B$ 可以被討論，則 $A\cup B$ 與 $A\cap B$ 也應該可以被討論。
</div>
...
這個例子說明，只要把一個事件放進事件集合族，就會被迫連同它所衍生出的基本事件一起放進去。
</div>

## 實數線上的 Borel $\sigma$-域

在機率論中，實數線 $\mathbb{R}$ 上最常使用的事件清單不是所有子集合，而是 Borel $\sigma$-域，記作 $\mathcal{B}(\mathbb{R})$。它可以由所有開區間生成，也可以由所有半直線 $(-\infty,x]$ 生成。

換句話說，$\mathcal{B}(\mathbb{R})$ 是一份足夠收納常見數線事件的清單。開區間、閉區間、半開區間、可數集合，以及它們經由可數聯集與可數交集形成的集合，都會被納入其中。

這個概念會在[隨機變數](/teaching-topics/random-variables-from-sample-space-to-real-line/)中再次出現。若 $X:S\to\mathbb{R}$ 是實值隨機變數，則每個 Borel 集合 $B\in\mathcal{B}(\mathbb{R})$ 的反映射 $X^{-1}(B)$ 都必須是 $\mathcal{F}$ 中的事件。如此一來，實數線上的集合才可被拉回樣本空間並賦予機率。
...
主題一將事件暫時理解為樣本空間的子集合；本篇則把這句話修正得更精確。
