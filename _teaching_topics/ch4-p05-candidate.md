...
topic: 5
order: 405
permalink: /teaching-topics/ch4-p05-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch4-p04-candidate/)由幾何級數出發，給出[幾何分配](/teaching-topics/ch4-p04-candidate/#def-geometric)的定義，證明了機率函數的合法性與期望值、變異數及動差母函數，接著給出[無記憶性](/teaching-topics/ch4-p04-candidate/#thm-memoryless)及其逆敘述，最後說明兩個獨立幾何變數取極小值之後仍為幾何分配。本篇不再新增性質，改以六道例題演練上述結果。
...
(2) 由[實驗幾何分配](/teaching-topics/ch4-p04-candidate/#def-geometric)的性質可知
{: .topic-paren-item}
...
$$
\begin{aligned}
&p_{\sssig X}(1)+p_{\sssig X}(3)+p_{\sssig X}(5)+\cdots\\[0.3em]
&\quad =p+p(1-p)^{2}+p(1-p)^{4}+\cdots\\[0.3em]
&\quad =\frac{p}{\,1-(1-p)^{2}\,}=\frac{1}{\,2-p\,}
\end{aligned}
$$
...
$$
\begin{aligned}
(N-1\mid X=0)&\sim\mathrm{Geo}\Bigl(p=\frac{2}{\,3\,}\Bigr)\ \Longrightarrow\ \mathbb{E}(N-1\mid X=0)=\frac{3}{\,2\,}\\[0.35em]
&\Longrightarrow\ \mathbb{E}(N\mid X=0)=\frac{\,5\,}{2}\\[0.7em]
(N-1\mid X=1)&\sim\mathrm{Geo}\Bigl(p=\frac{1}{\,3\,}\Bigr)\ \Longrightarrow\ \mathbb{E}(N-1\mid X=1)=3\\[0.35em]
...
$$
\begin{aligned}
&(N-1\mid X=0)\sim\mathrm{Geo}\Bigl(p=\frac{2}{\,3\,}\Bigr)\\[0.3em]
&\quad \Longrightarrow\ \mathbb{E}(N-1\mid X=0)=\frac{3}{\,2\,}\\[0.3em]
&\quad \Longrightarrow\ \mathbb{E}(N\mid X=0)=\frac{\,5\,}{2}
\end{aligned}
$$

$$
\begin{aligned}
&(N-1\mid X=1)\sim\mathrm{Geo}\Bigl(p=\frac{1}{\,3\,}\Bigr)\\[0.3em]
&\quad \Longrightarrow\ \mathbb{E}(N-1\mid X=1)=3\\[0.3em]
&\quad \Longrightarrow\ \mathbb{E}(N\mid X=1)=4
\end{aligned}
$$
...
<!-- ref-point: 待第三章第 12 篇 (雙重期望值定理，書稿 mathstatch3.tex 第 2267 行的
     Theorem 3.9，anchor 為 #thm-double-expectation) 發布後，將下一句的「雙重期望值」
     以及本篇小結第三段的「雙重期望值」改為指向該 anchor 的站內連結。 -->
...
&\quad +\mathbb{E}(N\mid X=1)\,\mathbb{P}(X=1)\\[0.3em]
&=\frac{\,5\,}{2}\times\frac{1}{\,3\,}+4\times\frac{2}{\,3\,}=\frac{\,7\,}{2}
\end{aligned}
$$
...
<!-- ref-point: 待第三章第 14 篇 (全機率定理，書稿 mathstatch3.tex 第 2512 行的
     Theorem 3.10，anchor 為 #thm-law-of-total-prob-r-v) 發布後，將下一句的「全機率定理」
     以及本篇小結第三段的「全機率定理」改為指向該 anchor 的站內連結。 -->
...
&\quad =\sum_{x=0}^{1}\mathbb{P}(N=4\mid X=x)\,\mathbb{P}(X=x)\\[0.3em]
&\quad =\mathbb{P}(N=4\mid X=0)\,\mathbb{P}(X=0)\\[0.3em]
&\qquad +\mathbb{P}(N=4\mid X=1)\,\mathbb{P}(X=1)\\[0.3em]
&\quad =\Bigl(\frac{1}{\,3\,}\Bigr)^{2}\Bigl(\frac{2}{\,3\,}\Bigr)\times\frac{1}{\,3\,}\\[0.3em]
...
<!-- ref-point: 待第三章第 15 篇 (變異數分解定理，書稿 mathstatch3.tex 第 2698 行的
     Theorem 3.11，anchor 為 #thm-var-decom-thm) 發布後，將下一句、本題之後那則 Note
     末句以及本篇小結第三段的「變異數分解定理」改為指向該 anchor 的站內連結。 -->
...
\begin{aligned}
&\mathrm{Var}(N)\\[0.3em]
&\quad =\mathbb{E}\bigl[\mathrm{Var}(N\mid X)\bigr]+\mathrm{Var}\bigl[\mathbb{E}(N\mid X)\bigr]\\[0.3em]
&\quad =\mathrm{Var}(N\mid X=0)\,\mathbb{P}(X=0)\\[0.3em]
&\qquad +\mathrm{Var}(N\mid X=1)\,\mathbb{P}(X=1)\\[0.3em]
&\qquad +\mathrm{Var}\bigl[\mathbb{E}(N\mid X)\bigr]\\[0.3em]
&\quad =\frac{\frac{1}{\,3\,}}{\,\bigl(\frac{2}{\,3\,}\bigr)^{2}\,}\times\frac{1}{\,3\,}+\frac{\frac{2}{\,3\,}}{\,\bigl(\frac{1}{\,3\,}\bigr)^{2}\,}\times\frac{2}{\,3\,}\\[0.3em]
...
<!-- ref-point: 待第三章第 11 篇 (條件期望值與條件變異數，書稿 mathstatch3.tex 第 2055 行的
     Definition 3.11 與第 2099 行的 Definition 3.13，anchor 分別為
     #def-conditional-expectation 與 #def-conditional-variance) 發布後，將下列 Note 中
     首見的「條件變異數」與「條件期望值」改為指向該兩個 anchor 的站內連結。 -->
...
而在 (3) 中，我們利用了 $\mathrm{Var}(N\mid X=x)=\mathrm{Var}(N-1\mid X=x)$ 的特性，快速地得到條件變異數，並計算條件變異數的期望值；再由 (1) 所計算的條件期望值，計算條件期望值的變異數，從而完成變異數分解定理。
...
(3) $X$ 與 $Y$ 的分配同為幾何分配，且參數皆為 <span class="text-nowrap">$p=1-p_s$，</span>此類分配不因過去曾成功完成的任務數 $k$ 而改變，稱為[無記憶性](/teaching-topics/ch4-p04-candidate/#thm-memoryless)。
{: .topic-paren-item}
...
[Example 4.15](#ex-geometric-ex-6) 記錄的是被擊落之前所能安全完成的任務數，值域自 $0$ 起算，屬於失敗幾何分配。已經安全完成 $k$ 次之後還能再完成幾次，其分配與一開始的 $X$ 完全相同，這正是[無記憶性](/teaching-topics/ch4-p04-candidate/#thm-memoryless)所述的性質。

[下一篇](/teaching-topics/ch4-p06-candidate/)把「做到第一次成功為止」改成「做到第 $r$ 次成功為止」，所需要的實驗次數即服從負二項分配；該篇先給出驗證機率函數時所需要的負二項級數。
