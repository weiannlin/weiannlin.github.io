...
topic: 6
order: 306
permalink: /teaching-topics/ch3-p06-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch3-p05-candidate/)以三道例題示範了聯合機率密度函數在指定區域上的積分，其中最後一題的三個變數都服從標準均勻分配，機率因而等於所求範圍在整個值域中所佔的比例。

本篇分成兩個部分，兩者所處理的對象並不相同。**前半是兩道序列型的例題**: 題目給的是一列 iid <span lang="en">(independent and identically distributed)</span> 標準均勻分配的隨機變數 $U_1,U_2,\ldots$ 這個序列，並以某個條件首次成立的下標定出一個隨機的 <span class="text-nowrap">$N$，</span>所求則是 $N$ 與序列本身的聯合機率。這兩題所用的工具正是上一篇的重積分，只是積分的維度由三維推廣到 $n$ 維，而題目本身的關鍵，則在於回到 $N$ 的定義去建構所求機率的範圍。**後半回到分配函數本身**: 二元隨機向量的聯合累積分配函數、聯合機率密度函數與邊際機率密度函數都已經定義過，還差邊際的累積分配函數尚未給出，本篇把它補上，並說明它與聯合累積分配函數之間的關係。
...
在此處中沿用了[前一道題](/teaching-topics/ch3-p05-candidate/#ex-three-iid-uniform-order)的計算結果，除此之外，讀者應該會對此處的做法感到熟悉，因為在 [Example 2.16](/teaching-topics/ch2-p207-candidate/#ex-car-offer-waiting-time) 中，我們就曾經依照 $N$ 的定義，以序列計算所求機率，甚至求出期望值。
...
(2) 由於 $\mathbb{P}(N>n)=\frac{1}{\,n!\,}$ 成立，故由 [Theorem 2.8](/teaching-topics/ch2-p06-candidate/#thm-expectation-tail-sum) 可知
{: .topic-paren-item}
...
以上兩題所處理的都是特定範圍上的機率。接下來我們把注意力移回分配函數本身: 二元隨機向量的[聯合累積分配函數](/teaching-topics/ch3-p02-candidate/#def-joint-cdf)、[聯合機率密度函數](/teaching-topics/ch3-p03-candidate/#def-joint-pdf)與[邊際機率密度函數](/teaching-topics/ch3-p04-candidate/#def-marginal-pdf)都已經給過定義，還差各個變數自己的累積分配函數尚未定義。
...
這個定義指出邊際 pdf (或 pmf) 的積分 (或加總) 即為邊際 cdf，應是一個相當符合直覺的定義。然而，這個定義若是與 [Definition 3.5](/teaching-topics/ch3-p03-candidate/#def-joint-pdf) 及 [Definition 3.6](/teaching-topics/ch3-p04-candidate/#def-marginal-pdf) 相結合，則可以得到以下這個有用的性質:
...
讀者如果確實將 [Example 3.2](/teaching-topics/ch3-p04-candidate/#ex-joint-pdf-region-basic) 至 [Example 3.4](/teaching-topics/ch3-p05-candidate/#ex-joint-pdf-exponential-region) 的積分範圍思考透徹，則應該發現，這些範圍上的積分結果，事實上都能夠與 [Theorem 3.2](#thm-joint-cdf-prob) 的結果相呼應。
...
回顧這一段的脈絡，我們先以 $\mathbb{P}(X\leqslant x,Y\leqslant y)$ 定出[聯合累積分配函數](/teaching-topics/ch3-p02-candidate/#def-joint-cdf)，再由二重積分的表示式定出[聯合機率密度函數](/teaching-topics/ch3-p03-candidate/#def-joint-pdf)，接著對其中一個變數積分而得到[邊際機率密度函數](/teaching-topics/ch3-p04-candidate/#def-marginal-pdf)，並以數道例題練習了[各種區域上的機率](/teaching-topics/ch3-p05-candidate/)如何求取，最後補上邊際累積分配函數，把四個機率函數與兩個分配函數之間的關係補齊。下一篇把隨機向量中的一個或多個變數固定成常數，看看這時候的機率分配會變成什麼樣子，此即條件機率分配。
