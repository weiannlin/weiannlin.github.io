...
[上一篇文章](/teaching-topics/probability-accumulates/)已經提過，有些隨機變數的機率是靠「加總」取得的。若隨機變數只會取到有限或可數無限多個值，則事件 $\{X\leq x\}$ 的機率，就是把不超過 $x$ 的那些單點機率加起來。
...
下一篇會轉向連續型隨機變數。當單點不再具有正機率時，機率不再逐點相加，而要改由密度函數在區間上的面積來計算。
