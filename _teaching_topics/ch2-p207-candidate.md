...
topic: 7
order: 207
permalink: /teaching-topics/ch2-p207-candidate/
date: 2026-08-06
published: false
...
[上一篇](/teaching-topics/ch2-p06-candidate/)給出期望值的定義，並以非負隨機變數的尾機率表示作結。
...
又 $N$ 為非負整數隨機變數，故由 [Theorem 2.8](/teaching-topics/ch2-p06-candidate/#thm-expectation-tail-sum) 可得
...
<!-- ref-point: 待第三章的獨立隨機變數主題寫到「同分配」並發布之後，將本句的「下一章」改為指向該篇的站內連結。 -->
iid 是我們在下一章才會提到的性質，是多個隨機變數彼此獨立 <span lang="en">(independent)</span> 且同分配 <span lang="en">(identically distributed)</span> 的意思，在此階段讀者不妨想像成，每個隨機變數不互相影響，且客觀條件皆相同。
...
離散型之下還可能沒有任何一個 $X_n$ 超過 $X_0$，此時 $N$ 取不到有限值，只要這件事發生的機率為正，$\mathbb{E}(N)=\infty$ 自然成立；在剛才那個離散型的例子裡，這個機率就是 $\mathbb{P}(X_0=1)=\frac{1}{2}$。共同的分配為連續型時 $\sum_{k=1}^{\infty}\frac{1}{k(k+1)}=1$，$N$ 取到有限值的機率為 $1$，解答中才能直接引用 [Theorem 2.8](/teaching-topics/ch2-p06-candidate/#thm-expectation-tail-sum)。
...
&\quad -\int_{-\infty}^{0}F_{\sssig X}(x)\,dx
\end{aligned}
$$
...
&\quad =\int_{0}^{\infty}xf_{\sssig X}(x)\,dx\\[0.2em]
&\qquad +\int_{-\infty}^{0}xf_{\sssig X}(x)\,dx\\[0.45em]
&\quad =\int_{0}^{\infty}\left(\int_{0}^{x}1\,dy\right)f_{\sssig X}(x)\,dx\\[0.2em]
&\qquad -\int_{-\infty}^{0}\left(\int_{0}^{-x}1\,dy\right)f_{\sssig X}(x)\,dx\\[0.45em]
&\quad =\int_{0}^{\infty}\int_{0}^{x}f_{\sssig X}(x)\,dy\,dx\\[0.2em]
&\qquad -\int_{-\infty}^{0}\int_{0}^{-x}f_{\sssig X}(x)\,dy\,dx\\[0.45em]
&\quad =\int_{0}^{\infty}\int_{y}^{\infty}f_{\sssig X}(x)\,dx\,dy\\[0.2em]
&\qquad -\int_{0}^{\infty}\int_{-\infty}^{-y}f_{\sssig X}(x)\,dx\,dy\\[0.45em]
&\quad =\int_{0}^{\infty}\bigl[1-F_{\sssig X}(y)\bigr]\,dy\\[0.2em]
&\qquad -\int_{0}^{\infty}\bigl[F_{\sssig X}(-y)-0\bigr]\,dy\\[0.45em]
&\quad =\int_{0}^{\infty}\bigl[1-F_{\sssig X}(y)\bigr]\,dy\\[0.2em]
&\qquad -\int_{-\infty}^{0}F_{\sssig X}(x)\,dx
\end{aligned}
$$
...
[Theorem 2.8](/teaching-topics/ch2-p06-candidate/#thm-expectation-tail-sum) 的連續型版本，在工業領域經常被用來計算產品的故障前平均時間 <span lang="en">(mean time to failure, MTTF)</span>，這個特別的數字其實就是期望值，但在只能失效一次且不可修復的系統中，這個指標具有極重要的意義，多數用於計算產品的平均壽命。
...
這個定理的計算細節，並不若我們的直覺這麼簡單，其牽涉到[隨機變數變換 <span lang="en">(transformation of random variable)</span>](/teaching-topics/ch2-p223-candidate/)，我們將在本章的最後詳談，讀者在此可以直覺地直接計算即可。
...
&\quad +\int_{1/2}^{1}x\cdot6\,x(1-x)\,dx\\[0.45em]
&=\frac{11}{16}
\end{aligned}
$$
...
事實上，我們曾在 [Definition 2.6](/teaching-topics/ch2-p06-candidate/#def-expectation) 之後提到，期望值 (特別是離散型) 的概念中，可以將 $x$ 類比為位置，而 $p_{\sssig X}(x)$ 類比為該點的質量，則期望值就是質心；這個想法若沿用至此，質點的位置經過實值可測函數 $g(\cdot)$ 的轉換之後，函數期望值就是轉換過後的質心位置，由此來思考這個定理，便會容易許多。
...
&\quad =\sum_{i=1}^{k}a_i\,\mathbb{E}\bigl[g_i(X)\bigr]+b
\end{aligned}
$$
...
\begin{aligned}
&\mathbb{E}\left[\sum_{i=1}^{k}a_i\,g_i(X)+b\right]\\[0.45em]
&\quad =\sum_{x\in\mathcal{R}_{\sssig X}}\left[\left(\sum_{i=1}^{k}a_i\,g_i(x)+b\right)p_{\sssig X}(x)\right]\\[0.45em]
&\quad =\sum_{x\in\mathcal{R}_{\sssig X}}\left[\sum_{i=1}^{k}a_i\,g_i(x)\,p_{\sssig X}(x)\right.\\[0.2em]
&\qquad \left.+\,b\,p_{\sssig X}(x)\right]\\[0.45em]
&\quad =\sum_{i=1}^{k}a_i\left[\sum_{x\in\mathcal{R}_{\sssig X}}g_i(x)\,p_{\sssig X}(x)\right]\\[0.2em]
&\qquad +\,b\sum_{x\in\mathcal{R}_{\sssig X}}p_{\sssig X}(x)\\[0.45em]
&\quad =\sum_{i=1}^{k}a_i\,\mathbb{E}\bigl[g_i(X)\bigr]+b
\end{aligned}
$$
...
這個性質與後續要提到的**變異數 <span lang="en">(variance)</span>** 的[**平方伸縮 (scaled by square)**](/teaching-topics/ch2-p208-candidate/#thm-variance-properties) 性質具有很大的區別，我們將在介紹變異數時一併詳談。
...
$$
\begin{aligned}
&\mathbb{E}\bigl[(X+2)^{2}\bigr]\\[0.45em]
&\quad =\int_{0}^{2}(x+2)^{2}\times\frac{x+2}{6}\,dx\\[0.45em]
&\quad =\frac{1}{6}\int_{0}^{2}(x+2)^{3}\,dx\\[0.45em]
&\quad =\frac{1}{6}\int_{0}^{2}(x+2)^{3}\,d(x+2)\\[0.45em]
&\quad =\frac{1}{6}\times\left[\frac{(x+2)^{4}}{4}\right]_{0}^{2}\\[0.45em]
&\quad =\frac{256}{24}-\frac{16}{24}=10
\end{aligned}
$$
