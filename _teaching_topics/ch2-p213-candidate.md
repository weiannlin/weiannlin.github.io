...
topic: 13
order: 213
permalink: /teaching-topics/ch2-p213-candidate/
date: 2026-08-06
published: false
...
[上一篇](/teaching-topics/ch2-p212-candidate/)介紹分位數，把中位數的想法推廣到任意的比例。到這裡為止，期望值、變異數、標準差、眾數、中位數與分位數都已經看過，它們各自描述一個分配的某一個面向，彼此之間看起來並沒有共同的來源。
...
(1) 上述定義中可以發現，母體一階原動差即為[期望值](/teaching-topics/ch2-p06-candidate/#def-expectation)；而母體二階主動差即為[變異數](/teaching-topics/ch2-p208-candidate/#def-variance)。
{: .topic-paren-item}
...
讀者應該記得，[期望值](/teaching-topics/ch2-p06-candidate/#def-expectation)與[變異數](/teaching-topics/ch2-p208-candidate/#def-variance)，在稍早曾被用來分別描述分配的中央趨勢與分散趨勢，這也是動差的功能之一。

</div>

(3) 稍後我們會看到[**動差母函數 <span lang="en">(moment generating function, mgf)</span>**](/teaching-topics/ch2-p215-candidate/#def-mgf)，這是一個能夠生成任意 $r$ 階**原動差**的函數。
{: .topic-paren-item}
...
&\quad =\frac{1}{\,n\,}\bigl[2^{k}+3^{k}+\cdots+(1+n)^{k}\bigr]\\[0.2em]
