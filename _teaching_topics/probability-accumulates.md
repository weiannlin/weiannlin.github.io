...
以下各節都以此定義為準。離散型與連續型的差別，不在於 CDF 的定義改變，而在於事件 $\lbrace X\leq x\rbrace$ 的機率如何被計算與讀出。
...
在 Example 2.1 中，$X$ 只能取 $1,2,\ldots,6$。這個集合是 $X$ 的**值域 (range)**。但 CDF 的輸入 $x$ 不是骰子點數本身，而是數線上的門檻，因此 $x$ 可以是任意實數。
...
另一種常見情形中，單點本身不承載正機率，機率沿著區間連續累積。若存在非負函數 $f_X$，使得對任意 $x\in\mathbb{R}$ 皆有
...
此時事件 $\lbrace X\leq x\rbrace$ 的機率不是靠單點相加，而是靠密度函數在 $(-\infty,x]$ 上的面積累積。
...
這不表示 $a$ 不可能成為觀測值，而是說單點在連續模型中沒有面積，故不承載正機率。這一點延續了第一章中[飛鏢落點的直覺校準](/teaching-topics/random-experiments-sample-space-events/#thought-experiment-dart-zero-probability)。也因此，對連續型隨機變數而言，端點是否包含通常不會改變區間機率。
...
因此，隨機變數不只是樣本點的重新命名；函數、極限、微分、積分與分佈形狀都能納入討論，後續的常見分配、期望值與極限定理也由此展開。
