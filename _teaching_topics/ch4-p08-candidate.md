...
topic: 8
order: 408
permalink: /teaching-topics/ch4-p08-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch4-p07-candidate/)介紹[超幾何分配](/teaching-topics/ch4-p07-candidate/#def-hypergeometric)，至此伯努利實驗相關的機率模型已經全部給出。這一類模型所記錄的，都是在指定次數的實驗之中成功發生了幾次，或者為了取得指定次數的成功而必須做幾次實驗。本篇轉入第二大類的機率模型，也就是由卜瓦松過程所衍生的各個分配。
...
$$
\begin{aligned}
&\mathbb{E}\bigl[X(X-1)\bigr]\\[0.4em]
&\quad =\sum_{x=0}^{\infty}x(x-1)\frac{e^{-\lambda}\lambda^{x}}{\,x!\,}\\[0.4em]
&\quad =\sum_{x=2}^{\infty}\frac{e^{-\lambda}\lambda^{x}}{\,(x-2)!\,}\\[0.4em]
&\quad =\lambda^{2}\sum_{x=2}^{\infty}\frac{e^{-\lambda}\lambda^{x-2}}{\,(x-2)!\,}\\[0.4em]
&\quad =\lambda^{2}e^{-\lambda}\sum_{y=0}^{\infty}\frac{\lambda^{y}}{\,y!\,}\\[0.4em]
...
第二，在[二項分配](/teaching-topics/ch4-p02-candidate/#def-binomial)中，若 $n\to\infty,\ p\to0$ 且 <span class="text-nowrap">$np=\lambda$，</span>則有
{: .topic-paren-cont}
...
<div class="topic-proof" markdown="1">
**Proof.** 令 <span class="text-nowrap">$X\sim\mathrm{Bin}(n,\ p)$，</span>若 $n\to\infty,\ p\to0$ 且 <span class="text-nowrap">$np=\lambda$，</span>則有

$$
p=\frac{\lambda}{\,n\,}
$$

則由[二項分配](/teaching-topics/ch4-p02-candidate/#def-binomial)的 pmf，我們有
...
&\quad =\lim_{n\to\infty}\frac{n!}{\,x!(n-x)!\,}\biggl(\frac{\lambda}{\,n\,}\biggr)^{x}\\[0.25em]
&\qquad\qquad \times\biggl(1-\frac{\lambda}{\,n\,}\biggr)^{n-x}\\[0.4em]
&\quad =\frac{\lambda^{x}}{\,x!\,}\times\lim_{n\to\infty}\frac{n!}{\,(n-x)!\,}\biggl(\frac{1}{\,n\,}\biggr)^{x}\\[0.25em]
&\qquad\qquad \times\biggl(1-\frac{\lambda}{\,n\,}\biggr)^{n-x}\\[0.4em]
&\quad =\frac{\lambda^{x}}{\,x!\,}\times\lim_{n\to\infty}\frac{n!}{\,n^{x}(n-x)!\,}\\[0.25em]
&\qquad\qquad \times\biggl(1-\frac{\lambda}{\,n\,}\biggr)^{n}\biggl(1-\frac{\lambda}{\,n\,}\biggr)^{-x}\\[0.4em]
&\quad =\frac{\lambda^{x}}{\,x!\,}\times\lim_{n\to\infty}\frac{n\times\cdots\times(n-x+1)}{\,n\times\cdots\times n\,}\\[0.25em]
&\qquad\qquad \times\biggl(1-\frac{\lambda}{\,n\,}\biggr)^{n}\biggl(1-\frac{\lambda}{\,n\,}\biggr)^{-x}\\[0.4em]
&\quad =\frac{\lambda^{x}}{\,x!\,}\times1\times e^{-\lambda}\times1\\[0.25em]
&\quad =\frac{e^{-\lambda}\lambda^{x}}{\,x!\,},\ x=0,1,\ldots,\infty
\end{aligned}
$$
