...
topic: 12
order: 412
permalink: /teaching-topics/ch4-p12-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch4-p11-candidate/)談的是指數分配的無記憶性，以及兩個獨立指數分配的極小值與先後次序。指數分配所描述的，是[卜瓦松過程](/teaching-topics/ch4-p08-candidate/#def-poisson-process)中等候直到一次偶發事件發生所需的等待時間；若把「一次」換成「$n$ 次」，這段等待時間所服從的分配，就是本篇的伽瑪分配。

本篇先給出伽瑪分配的定義，並完整證明其機率函數為一個合法的機率函數，以及期望值、變異數與動差母函數的公式，另外再求出 $k$ 階原動差的公式。接著依序說明四項延伸性質: 伽瑪分配與[卜瓦松分配](/teaching-topics/ch4-p08-candidate/#def-poisson-distribution)的對偶關係、形狀參數為 $1$ 時即為[指數分配](/teaching-topics/ch4-p10-candidate/#def-exponential-distribution)、比例參數相同且彼此獨立的兩個伽瑪變數相加仍為伽瑪分配，以及比例伸縮性質。最後以四道例題作為演練。
...
&\quad =\int_{0}^{\infty}\frac{1}{\,\beta^{\alpha}\Gamma(\alpha)\,}x^{\alpha-1}e^{-\frac{x}{\,\beta\,}}\,dx\\[0.45em]
&\quad =\frac{\,\beta^{\alpha}\Gamma(\alpha)\,}{\beta^{\alpha}\Gamma(\alpha)}=1
\end{aligned}
$$
...
\alpha,\beta>0,\ k\in\bigl(\lbrace0\rbrace\cup\mathbb{N}\bigr)
\end{gathered}
$$
...
$$
\begin{aligned}
&\mathbb{E}\bigl(X^{k}\bigr)\\[0.35em]
&\quad =\int_{0}^{\infty}x^{k}\cdot\frac{1}{\,\beta^{\alpha}\Gamma(\alpha)\,}x^{\alpha-1}e^{-\frac{x}{\beta}}\,dx\\[0.45em]
&\quad =\frac{1}{\,\beta^{\alpha}\Gamma(\alpha)\,}\int_{0}^{\infty}x^{\alpha+k-1}e^{-\frac{x}{\beta}}\,dx\\[0.45em]
&\quad =\frac{1}{\,\beta^{\alpha}\Gamma(\alpha)\,}\beta^{\alpha+k}\Gamma(\alpha+k)\\[0.45em]
&\quad =\beta^{k}\frac{\,\Gamma(\alpha+k)\,}{\Gamma(\alpha)},\\[0.25em]
...
&\quad -\sum_{y=1}^{n-1}\left[\frac{d}{d\,t}\left(\frac{e^{-\lambda t}(\lambda t)^{y}}{y!}\right)\right]\\[0.45em]
&=\lambda e^{-\lambda t}\\[0.25em]
&\quad -\sum_{y=1}^{n-1}\left[\frac{\,-\lambda e^{-\lambda t}(\lambda t)^{y}\,}{y!}\right.\\[0.25em]
...
&\quad -\lambda\sum_{y=1}^{n-1}\frac{\,e^{-\lambda t}\,(\lambda t)^{y-1}\,}{(y-1)!}\\[0.45em]
&=\lambda e^{-\lambda t}+\lambda\sum_{y=1}^{n-1}\frac{\,e^{-\lambda t}(\lambda t)^{y}\,}{y!}\\[0.25em]
&\quad -\lambda\sum_{k=0}^{n-2}\frac{\,e^{-\lambda t}\,(\lambda t)^{k}\,}{k!}\\[0.25em]
...
<div class="topic-proof" markdown="1">
**Proof.** 由 $X\sim\mathrm{Gamma}(\alpha,\ \beta)$ 可知
...
$$
M_{\sssig X}(t)=(1-\beta t)^{-1},\ t<\frac{1}{\,\beta\,}\ \Longrightarrow\ X\sim\mathrm{Exp}(\beta)
$$
...
\Longrightarrow\ X\sim\mathrm{Exp}(\beta)
\end{gathered}
$$
...
<!-- ref-point: 待第三章第 25 篇 (獨立隨機變數的線性組合之動差母函數，書稿 mathstatch3.tex
     ...
     本篇另有一處同樣待回填，見 Example 4.34 的兩個小題。 -->
...
&\quad =\int_{0}^{x}\frac{\,t^{\alpha-1}e^{\frac{-t}{\beta}}\,}{\,\beta^{\alpha}\Gamma(\alpha)\,}\,dt\\[0.45em]
&\quad =\frac{1}{\,\beta^{\alpha}\Gamma(\alpha)\,}\int_{0}^{x}t^{\alpha-1}e^{\frac{-t}{\beta}}\,dt\\[0.45em]
&\quad =\frac{1}{\,\beta^{\alpha}\alpha\Gamma(\alpha)\,}\int_{0}^{x}e^{\frac{-t}{\beta}}\,d\bigl(t^{\alpha}\bigr)\\[0.45em]
&\quad =\frac{1}{\,\beta^{\alpha}\Gamma(\alpha+1)\,}\Bigl[\Bigl(e^{\frac{-t}{\beta}}t^{\alpha}\Bigr)\Big|^{x}_{0}\\[0.25em]
&\qquad\qquad -\int_{0}^{x}t^{\alpha}\,d\bigl(e^{\frac{-t}{\beta}}\bigr)\Bigr]\\[0.45em]
&\quad =\frac{\,e^{\frac{-x}{\beta}}\bigl(\frac{x}{\beta}\bigr)^{\alpha}\,}{\,\alpha!\,}\\[0.25em]
...
&\quad =\frac{\,e^{\frac{-x}{\beta}}\bigl(\frac{x}{\beta}\bigr)^{\alpha}\,}{\,\alpha!\,}+h(\alpha+1)\\[0.45em]
&\quad =\frac{\,e^{\frac{-x}{\beta}}\bigl(\frac{x}{\beta}\bigr)^{\alpha}\,}{\,\alpha!\,}+\frac{\,e^{\frac{-x}{\beta}}\bigl(\frac{x}{\beta}\bigr)^{\alpha+1}\,}{\,(\alpha+1)!\,}\\[0.25em]
&\qquad +h(\alpha+2)=\ldots\\[0.45em]
&\quad =\sum_{y=\alpha}^{\infty}\frac{\,e^{\frac{-x}{\beta}}\bigl(\frac{x}{\beta}\bigr)^{y}\,}{\,y!\,}=\mathbb{P}(Y\geqslant\alpha)
\end{aligned}
$$
...
&\quad =1-\mathbb{P}(Y=0)-\mathbb{P}(Y=1)\\[0.45em]
&\quad =1-(1+10\lambda)e^{-10\lambda}
\end{aligned}
$$
...
&=\lambda e^{-\lambda t}\\[0.25em]
&\quad -\sum_{y=1}^{k-1}\left[\frac{d}{d\,t}\left(\frac{e^{-\lambda t}(\lambda t)^{y}}{y!}\right)\right]\\[0.45em]
&=\lambda e^{-\lambda t}\\[0.25em]
&\quad -\sum_{y=1}^{k-1}\left[\frac{\,-\lambda e^{-\lambda t}(\lambda t)^{y}\,}{y!}\right.\\[0.25em]
&\qquad\qquad\left.+\frac{\,e^{-\lambda t}\,y\,(\lambda t)^{y-1}\,\lambda\,}{y!}\right]\\[0.45em]
&=\lambda e^{-\lambda t}+\lambda\sum_{y=1}^{k-1}\frac{\,e^{-\lambda t}(\lambda t)^{y}\,}{y!}\\[0.25em]
&\quad -\lambda\sum_{y=1}^{k-1}\frac{\,e^{-\lambda t}\,(\lambda t)^{y-1}\,}{(y-1)!}\\[0.45em]
&=\lambda e^{-\lambda t}+\lambda\sum_{y=1}^{k-1}\frac{\,e^{-\lambda t}(\lambda t)^{y}\,}{y!}\\[0.25em]
&\quad -\lambda\sum_{j=0}^{k-2}\frac{\,e^{-\lambda t}\,(\lambda t)^{j}\,}{j!}\\[0.25em]
...
[下一篇](/teaching-topics/ch4-p13-candidate/)介紹韋伯分配，並依序給出可靠度函數與風險函數的定義。
