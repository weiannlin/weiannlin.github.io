...
topic: 10
order: 310
permalink: /teaching-topics/ch3-p10-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch3-p09-candidate/)先以 [Theorem 3.3](/teaching-topics/ch3-p09-candidate/#thm-multiplication-rule-r-v) 說明聯合機率函數如何由條件機率函數與邊際機率函數相乘而得，再以 [Definition 3.10](/teaching-topics/ch3-p09-candidate/#def-indep-r-v) 給出獨立隨機變數的定義。本篇把期望值推廣到二元的情形。

單變數的時候，[Theorem 2.9](/teaching-topics/ch2-p207-candidate/#thm-expectation-of-function) 說明 $g(X)$ 的期望值如何由 $X$ 的機率函數求得，[Theorem 2.10](/teaching-topics/ch2-p207-candidate/#thm-expectation-linearity) 則給出期望值與線性組合可以交換的性質。本篇的四個定理依同樣的次序排列: 先說明二元的期望值如何由[聯合機率質量函數](/teaching-topics/ch3-p01-candidate/#def-joint-pmf)或[聯合機率密度函數](/teaching-topics/ch3-p03-candidate/#def-joint-pdf)求得，再給出線性組合的性質，接著處理相乘這種非線性的設定在兩個變數獨立之下的特例，最後把變異數的定義一併推廣。
...
$$
\begin{aligned}
&\mathbb{E}\bigl[g(X,Y)\bigr]\\[0.45em]
&\quad =\iint_{(x,y)\in\mathcal{R}_{\sssig XY}}g(x,y)\,f_{\sssig XY}(x,y)\,dx\,dy
\end{aligned}
$$
...
(1) 與 [Theorem 2.9](/teaching-topics/ch2-p207-candidate/#thm-expectation-of-function) 相同，這裡的 $g(\cdot, \cdot)$ 可以是任意的實值函數，一旦我們設定了 $g(X, Y) = X$ 或 $g(X, Y) = Y$ 這樣的函數，則 [Theorem 3.4](#thm-multi-exp) 的期望值將會變成 $\mathbb{E}(X)$ 與 <span class="text-nowrap">$\mathbb{E}(Y)$。</span>
{: .topic-paren-item}
...
\begin{aligned}
&\mathbb{E}(X)=\iint_{(x,y)\in\mathcal{R}_{\sssig XY}}x\,f_{\sssig XY}(x,y)\,dx\,dy\\[0.45em]
&\quad =\iint_{(x,y)\in\mathcal{R}_{\sssig XY}}x\,f_{\sssig XY}(x,y)\,dy\,dx\\[0.45em]
&\quad =\int_{x\in\mathcal{R}_{\sssig X}}x\Biggl[\int_{y\in\mathcal{R}_{\sssig Y}}f_{\sssig XY}(x,y)\,dy\Biggr]\,dx\\[0.45em]
&\quad =\int_{x\in\mathcal{R}_{\sssig X}}x\Bigl[f_{\sssig X}(x)\Bigr]\,dx
\end{aligned}
$$
...
(2) 有了多元隨機變數的期望值，我們便能夠進行許多運算，但我們應該先關心的是，如同 [Theorem 2.9](/teaching-topics/ch2-p207-candidate/#thm-expectation-of-function) 與 [Theorem 2.10](/teaching-topics/ch2-p207-candidate/#thm-expectation-linearity) 的關係一般，[Theorem 3.4](#thm-multi-exp) 也有類似 [Theorem 2.10](/teaching-topics/ch2-p207-candidate/#thm-expectation-linearity) 的性質，下面馬上就來看看這個性質。
{: .topic-paren-item}
...
$$
\begin{aligned}
&\mathbb{E}\biggl[\sum_{i=1}^{k}a_i\,g_i(X,Y)+b\biggr]\\[0.45em]
&\quad =\sum_{i=1}^{k}a_i\,\mathbb{E}\bigl[g_i(X,Y)\bigr]+b
\end{aligned}
$$
...
$$
\begin{aligned}
&\mathbb{E}\bigl[g(X)h(Y)\bigr]\\[0.45em]
&\quad =\int_{-\infty}^{\infty}\int_{-\infty}^{\infty}g(x)\,h(y)\,f_{\sssig XY}(x, y)\,dx\,dy\\[0.45em]
&\quad =\int_{-\infty}^{\infty}\int_{-\infty}^{\infty}g(x)\,h(y)\,f_{\sssig X}(x)\,f_{\sssig Y}(y)\,dx\,dy\\[0.45em]
&\quad =\int_{-\infty}^{\infty}g(x)\,f_{\sssig X}(x)\,dx\,\int_{-\infty}^{\infty}h(y)\,f_{\sssig Y}(y)\,dy\\[0.45em]
&\quad =\mathbb{E}\bigl[g(X)\bigr]\,\mathbb{E}\bigl[h(Y)\bigr]
\end{aligned}
$$
...
稍後我們會看見，$X$ $\indep$ $Y$ $\Longrightarrow$ $\mathbb{E}(XY)$ $=$ <span class="text-nowrap">$\mathbb{E}(X)\mathbb{E}(Y)$，</span>此即 $X$ 與 $Y$ 獨立導致 $X$ 與 $Y$ **零相關 <span lang="en">(uncorrelated)</span>**，但這個敘述的逆命題一般而言卻是不對的。

</div>

<!-- ref-point: 待第三章篇 18 的共變異數與零相關發布後，將本則 Note 的「零相關」改為指向該處的站內連結。 -->

上述定理常見的用途在於，尋找 $X$ 與 $Y$ 的**交叉動差 (cross moment)**，即 <span class="text-nowrap">$\mathbb{E}(XY)$，</span>在 $X$ $\indep$ $Y$ 的狀況下，$\mathbb{E}(XY)$ $=$ <span class="text-nowrap">$\mathbb{E}(X)\mathbb{E}(Y)$。</span>

<div class="topic-box topic-box--note" markdown="1">
<div class="topic-box__label">Note</div>

這個特例在稍後的小節介紹**共變異數 <span lang="en">(covariance)</span>** 的時候會談到。

</div>

<!-- ref-point: 待第三章篇 17 的 Definition 3.14 (交叉動差) 與篇 18 的 Definition 3.16 (共變異數) 發布後，將上一段的「交叉動差」與本則 Note 的「共變異數」改為站內連結。 -->
...
$$
\begin{aligned}
&\mathrm{Var}\bigl[g(X, Y)\bigr]\\[0.45em]
&\quad =\mathbb{E}\Bigl[\bigl(g(X, Y)-\mathbb{E}\bigl[g(X, Y)\bigr]\bigr)^{2}\Bigr]
\end{aligned}
$$
...
這個定理事實上就是 [Theorem 2.12](/teaching-topics/ch2-p208-candidate/#thm-variance-of-function) 的延伸版本。
...
$$
\mathbb{E}\bigl(X^2\bigr)-\bigl[\mathbb{E}(X)\bigr]^{2}=0\ \Longrightarrow\ \mathbb{E}\bigl(X^2\bigr)=\mathbb{E}(X)=0
$$
...
$$
\begin{gathered}
\mathbb{E}\bigl(X^2\bigr)-\bigl[\mathbb{E}(X)\bigr]^{2}=0\\[0.45em]
\Longrightarrow\ \mathbb{E}\bigl(X^2\bigr)=\mathbb{E}(X)=0
\end{gathered}
$$
...
&\quad =\mathbb{E}\bigl[X^2Y^2\bigr]-\bigl[\mathbb{E}(X)\mathbb{E}(Y)\bigr]^{2}\\[0.45em]
&\quad =\mathbb{E}\bigl(X^2\bigr)\,\mathbb{E}\bigl(Y^2\bigr)-\bigl[\mathbb{E}(X)\bigr]^{2}\,\bigl[\mathbb{E}(Y)\bigr]^{2}\\[0.45em]
&\quad =0\times\mathbb{E}\bigl(Y^2\bigr)-0^2\times\bigl[\mathbb{E}(Y)\bigr]^{2}=0
\end{aligned}
$$
...
在[第二章談變異數的性質](/teaching-topics/ch2-p208-candidate/#thm-variance-properties)中，我們曾經提過退化隨機變數 <span lang="en">(degenerate random variable)</span>，此處的 $X$ 正是這個概念。直觀上而言，讀者不妨將其視為一個常數 <span class="text-nowrap">$0$，</span>如此一來，這個問題就幾乎等同於在問 <span class="text-nowrap">$\mathrm{Var}(0\,Y)$，</span>其結果便顯得很直觀。
...
&\mathrm{Var}(XY)=\mathbb{E}\bigl[(XY)^2\bigr]-\bigl[\mathbb{E}(XY)\bigr]^{2}\\[0.45em]
&\quad =\mathbb{E}\bigl[X^2Y^2\bigr]-\bigl[\mathbb{E}(X)\mathbb{E}(Y)\bigr]^{2}\\[0.45em]
&\quad =(\sigma^2_1+\mu_1^2)(\sigma_2^2+\mu_2^2)-(\mu_1\,\mu_2)^{2}\\[0.45em]
&\quad =\mu_1^2\,\sigma_2^2+\mu_2^2\,\sigma_1^2+\sigma_1^2\,\sigma_2^2
\end{aligned}
$$
...
<div lang="en" markdown="1">
Suppose that $n$ balls are placed at random into $r$ boxes. Let $X_i$ take the value $1$ when box $i$ receives no ball, and the value $0$ otherwise.
...
&\quad =\mathbb{P}(X_i=1,X_j=1)=\frac{\,(r-2)^n\,}{r^n}
\end{aligned}
$$
...
&\quad =\mathbb{E}\!\left(\sum_{i=1}^{r}X_i^2+\mathop{\sum\sum}\limits_{i\neq j}X_i\,X_j\right)\\[0.45em]
&\quad =\sum_{i=1}^{r}\mathbb{E}(X_i^2)+2\mathop{\sum\sum}\limits_{i<j}\mathbb{E}(X_i\,X_j)\\[0.45em]
&\quad =r\left[\frac{\,(r-1)^n\,}{r^n}\right]\\[0.45em]
...
&\quad +r(r-1)\left[\frac{\,(r-2)^n\,}{r^n}\right]\\[0.45em]
&\quad -\left[\frac{\,(r-1)^n\,}{r^{n-1}}\right]^{2}
\end{aligned}
$$
...
&\quad +\sum_{i=1}^{4}\frac{\partial}{\,\partial X_i\,}f(\boldsymbol{X})\biggr\rvert_{\boldsymbol{X}=\boldsymbol{\mu}}(X_i-\mu_{\sssig X_i})
\end{aligned}
$$
...
&\quad +\sum_{i=1}^{4}\frac{\partial}{\,\partial X_i\,}f(\boldsymbol{X})\biggr\rvert_{\boldsymbol{X}=\boldsymbol{\mu}}(X_i-\mu_{\sssig X_i})\Biggr]\\[0.45em]
&\quad =\sum_{i=1}^{4}\Biggl[\frac{\partial}{\,\partial X_i\,}f(\boldsymbol{X})\biggr\rvert_{\boldsymbol{X}=\boldsymbol{\mu}}\Biggr]^{2}\sigma_{\sssig X_i}^{2}
\end{aligned}
$$
...
&\quad +\Bigl(\frac{1}{4}\mu_{\sssig X_1}\mu_{\sssig X_3}^{3}\mu_{\sssig X_4}^{-3}\Bigr)^{2}\sigma_{\sssig X_2}^{2}\\[0.45em]
&\quad +\Bigl(\frac{3}{4}\mu_{\sssig X_1}\mu_{\sssig X_2}\mu_{\sssig X_3}^{2}\mu_{\sssig X_4}^{-3}\Bigr)^{2}\sigma_{\sssig X_3}^{2}\\[0.45em]
&\quad +\Bigl(\frac{-3}{4}\mu_{\sssig X_1}\mu_{\sssig X_2}\mu_{\sssig X_3}^{3}\mu_{\sssig X_4}^{-4}\Bigr)^{2}\sigma_{\sssig X_4}^{2}
\end{aligned}
$$
...
上述問題事實上是 [Theorem 2.15](/teaching-topics/ch2-p209-candidate/#thm-taylor-approximation) 的延伸，只是此處由於變數變多了，故將微分改為偏微分，其他原理皆相同。
...
[下一篇](/teaching-topics/ch3-p11-candidate/)把期望值放到條件分配之上，介紹條件期望值與條件變異數。
