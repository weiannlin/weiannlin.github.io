...
第三章把隨機變數的概念推廣到[隨機向量](/teaching-topics/ch3-p01-candidate/#def-random-vector)，處理的是多個隨機變數合在一起時的聯合分配，以及變數與變數之間的關係。第一章到第三章所建立的，都是機率與隨機變數的一般性定義與性質；本章要談的則是幾個特別常見的機率分配模型。
...
事實上，伯努利分配的所有階原動差都是成功機率 <span class="text-nowrap">$p$，</span>並且這一點在 [Example 2.32 <span lang="en">(Continued)</span>](/teaching-topics/ch2-p216-candidate/#ex-constant-moments-bernoulli) 曾出現過。
