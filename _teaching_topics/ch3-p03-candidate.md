...
topic: 3
order: 303
permalink: /teaching-topics/ch3-p03-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch3-p02-candidate/)以 $\mathbb{P}(X\leqslant x,Y\leqslant y)$ 定義了聯合累積分配函數，並說明二元離散型隨機向量的聯合累積分配函數，是把聯合機率質量函數在指定範圍中做兩重的加總。二元連續型隨機向量沒有辦法以加總處理，須如同單變數的情形改用積分。
...
joint pdf 的定義，與[單變數的 pdf](/teaching-topics/ch2-p03-candidate/#def-pdf) 仍非常相似，僅是將單變數的函數推廣至雙變數的函數而已，因此一些 pdf 具備的特色在 joint pdf 身上亦可以看見，其中當然包含了 **joint pdf 不是機率**這件事情。
...
<figure id="fig-joint-pdf-surface" class="topic-figure topic-figure--wide">
  <img src="/images/teaching-topics/joint-pdf-surface.png" alt="一個聯合機率密度函數的曲面，中央隆起成鐘形，四周向外遞減趨近於零。底面兩軸分別標 x 與 y，縱軸標聯合機率密度函數的函數值。">
  <figcaption><span class="topic-figure__label">Fig. 3.2.</span> 一個很經典的 joint pdf。曲面在中央隆起，四周遞減；曲面之下所夾的體積即機率。</figcaption>
</figure>

前述提到的 joint cdf，即是在 [Fig. 3.1](/teaching-topics/ch3-p02-candidate/#fig-joint-cdf-quadrants) 所提到 joint cdf 的範圍內，[Fig. 3.2](#fig-joint-pdf-surface) 的曲面下所涵蓋的**體積**。若由微積分的觀點理解之，則我們能夠更清楚 joint pdf 與 joint cdf 間的關係。

當然，如同 [Fig. 2.4](/teaching-topics/ch2-p03-candidate/#fig-cdf-as-area) 曾以微積分的觀點解釋 pdf，此處亦可以同樣的觀點得到以下的關係式:
...
\left\lbrace
\begin{array}{l}
\dfrac{\partial^{2}F_{\sssig XY}(x,y)}{\partial x\,\partial y},\\[0.3em]
\quad\text{$(x,y)$ 是 $\mathcal{R}_{\sssig XY}$ 中的可微分點}\\[0.8em]
0,\\[0.3em]
\quad\text{$(x,y)$ 是 $\mathcal{R}_{\sssig XY}$ 中的不可微分點}\\[0.3em]
\quad\text{或不在 $\mathcal{R}_{\sssig XY}$ 中}
\end{array}
\right.
$$
...
&\quad =\iint_{(x,y)\in\mathcal{R}_{\sssig XY}}f_{\sssig XY}(x,y)\,dx\,dy\\[0.45em]
&\quad =\int_{-\infty}^{\infty}\int_{-\infty}^{\infty}f_{\sssig XY}(x,y)\,dx\,dy=1
\end{aligned}
$$
</div>
  </li>
...
(2) 性質 (1) 與 (2) 則是繼承了[單變數的 pdf 該有的性質](/teaching-topics/ch2-p03-candidate/#thm-pdf-properties)，只是在性質 (2) 的幾何意義上，則是由曲線下的面積總和為 <span class="text-nowrap">$1$，</span>變為**曲面下的體積總和為 $1$** 這件事情。另外還有許多相同之處，像是**積分範圍是否包含邊界點並不影響結果**與**單點積分仍然為 $0$** 等性質。
{: .topic-paren-item}
...
讀者可以參照 [Fig. 2.7](/teaching-topics/ch2-p04-candidate/#fig-single-point-zero) 的示意圖，當時我們所指示的機率，若由積分的角度而言，事實上是指 $X=a$ 所在的線段，而這個線段是沒有面積的，故當然沒有機率；此處只是將其往更高一個維度推廣而已。
...
$$
\begin{aligned}
&\mathbb{P}\bigl((X,Y)\in A\bigr)\\[0.45em]
&\quad =\mathbb{P}\bigl(a\leqslant X\leqslant b,\ c\leqslant Y\leqslant d\bigr)\\[0.45em]
&\quad =\int_{c}^{d}\int_{a}^{b}f_{\sssig XY}(x,y)\,dx\,dy\\[0.45em]
&\quad =\int_{a}^{b}\int_{c}^{d}f_{\sssig XY}(x,y)\,dy\,dx
\end{aligned}
$$
