...
topic: 24
order: 224
permalink: /teaching-topics/ch2-p224-candidate/
date: 2026-08-06
published: false
...
[上一篇](/teaching-topics/ch2-p223-candidate/)介紹隨機變數的函數轉換，離散型有直接列表法、pmf 法與 mgf 法，連續型有 cdf 法、Jacobian 法與 mgf 法。該篇所處理的 $Y=2X+1$、$Y=\frac{1}{X}$ 與 $Y=\bigl(1+e^{-X}\bigr)^{-1}$，其中的 $g(\cdot)$ 都是一對一函數，反函數直接存在，[pmf 法](/teaching-topics/ch2-p223-candidate/#prop-discrete-transformation-pmf)、[cdf 法](/teaching-topics/ch2-p223-candidate/#prop-cdf-method)與 [Jacobian 法](/teaching-topics/ch2-p223-candidate/#prop-jacobian-method)的公式都能直接套用。

然而 $g(\cdot)$ 的反函數並不是任何時候都存在。該篇在 [Jacobian 法](/teaching-topics/ch2-p223-candidate/#prop-jacobian-method)之後已經先交代過這時候的作法: 我們只需將原先的隨機變數分段，直到該段中 $g(\cdot)$ 的反函數存在，分段轉換為新變數的 pdf 後，再以分段隨機變數的概念將各段連接起來即可。本篇的三道例題都要用到這個作法，$Y=Z^{2}$、$Y\_{i}=\lvert X\_{i}\rvert$ 與 $Y=X^{2}$ 都不是一對一的轉換，每一題都以 cdf 法與 Jacobian 法各解一次: Jacobian 法正是分段之後再相加，cdf 法則不必先取反函數，直接由 cdf 的定義把 $\lbrace Y\leqslant y\rbrace$ 改寫成 $X$ 的事件即可。
...
第二章到此結束。全章先由樣本空間上的實值函數定出[隨機變數](/teaching-topics/ch2-p01-candidate/)，再以 pmf、[cdf](/teaching-topics/ch2-p02-candidate/) 與 [pdf](/teaching-topics/ch2-p03-candidate/) 描述它的機率分配，接著[由 cdf 計算各種區間的機率](/teaching-topics/ch2-p04-candidate/)，並處理離散型與連續型兼具的[混合型](/teaching-topics/ch2-p05-candidate/)。有了分配之後，[期望值](/teaching-topics/ch2-p06-candidate/)、[變異數](/teaching-topics/ch2-p208-candidate/)、[標準差](/teaching-topics/ch2-p209-candidate/)、[眾數](/teaching-topics/ch2-p210-candidate/)、[中位數](/teaching-topics/ch2-p211-candidate/)與[分位數](/teaching-topics/ch2-p212-candidate/)各自摘要分配的一個面向，[函數期望值與期望值的性質](/teaching-topics/ch2-p207-candidate/)一篇則回頭處理期望值本身，說明 $\mathbb{E}[g(X)]$ 不必先求出 $g(X)$ 的分配，直接由 $X$ 的分配即可算得，期望值對線性函數也具有可交換性；[動差系統](/teaching-topics/ch2-p213-candidate/)把其中幾個量數收進同一套架構之下，[形狀量數](/teaching-topics/ch2-p214-candidate/)則由三階與四階動差刻畫偏斜的方向與尾巴的厚薄。[動差母函數](/teaching-topics/ch2-p215-candidate/)換了一個角度: 以一個 $t$ 的函數同時生成各階原動差，並在[唯一性](/teaching-topics/ch2-p216-candidate/#thm-mgf-uniqueness)之下反過來認出分配是哪一個；[機率母函數與累積量母函數](/teaching-topics/ch2-p217-candidate/)則一個把工具函數換成 $t^{X}$、一個由 mgf 取對數，分別生成階乘動差與累積量，[特徵函數](/teaching-topics/ch2-p218-candidate/)再把工具函數換成 $e^{itX}$，因而對每一個隨機變數都存在。機率不等式及相關法則一節回到機率本身: [馬可夫、柴比雪夫與坎特利不等式](/teaching-topics/ch2-p219-candidate/)只用一階與二階動差就替尾機率取得上界，[車諾夫不等式](/teaching-topics/ch2-p220-candidate/)改用整個 mgf 而取得更緊的上界，[延森不等式](/teaching-topics/ch2-p221-candidate/#thm-jensen)所比較的則是 $\mathbb{E}[g(X)]$ 與 $g[\mathbb{E}(X)]$ 兩者的大小，而[鐘形分配經驗法則](/teaching-topics/ch2-p222-candidate/#thm-empirical-rule)改由分配的形狀直接說出中央區間的機率。最後這兩篇處理函數轉換，回答已知 $X$ 的分配時 $Y=g(X)$ 的分配要怎麼求: [一對一時直接套公式](/teaching-topics/ch2-p223-candidate/)，不是一對一時先分段再合併。第三章把討論的對象由一個隨機變數推廣到多個隨機變數所組成的隨機向量。
