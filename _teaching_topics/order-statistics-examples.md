...
[上一篇](/teaching-topics/order-statistics-distributions/)以 [Theorem 3.24](/teaching-topics/order-statistics-distributions/#thm-order-stat-samp-dist-pdf) 給出隨機樣本之[順序統計量](/teaching-topics/order-statistics/#def-order-stat)的抽樣分配 pdf，並以幾張圖說明其中的排列組合是怎麼數出來的，最後指出標準均勻分配的順序統計量都是貝塔分配。

本篇先以 [Example 3.63](#ex-uniform-order-statistics-five) 把這五款公式在標準均勻分配上實際用過一遍，一併求出全距的分配，並在其後說明順序統計量還可以定義出樣本全距與樣本[中位數](/teaching-topics/median/#def-median)這一類的統計量；接著以 [Example 3.64](#ex-three-iid-order-pdf) 由指數分配的隨機樣本求最小值與最大值的分配。之後以 [Theorem 3.25](#thm-order-stat-samp-dist-cdf) 給出順序統計量的抽樣分配 cdf 並證明之，最後以 [Example 3.65](#ex-discrete-order-statistics-die) 與 [Example 3.66](#ex-order-statistics-minimum) 兩道離散型的例題作結。
...
$$
\begin{aligned}
&\Longrightarrow f_{\sssig Y_1\cdots Y_5}(y_1,y_2,y_3,y_4,y_5)\\[0.45em]
&\quad =5!\times f_{\sssig X}(y_1)\times f_{\sssig X}(y_2)\\[0.2em]
&\qquad\quad\times f_{\sssig X}(y_3)\times f_{\sssig X}(y_4)\times f_{\sssig X}(y_5)\\[0.45em]
&\quad =5!\times 1^{5}=120\\[0.45em]
&\qquad 0<y_1<y_2<y_3<y_4<y_5<1
\end{aligned}
$$
...
\begin{aligned}
&f_{\sssig Y_1}(y_1)=5\,f_{\sssig X}(y_1)\bigl[1-F_{\sssig X}(y_1)\bigr]^{5-1}\\[0.45em]
&\quad =5\times 1\times\bigl[1-y_1\bigr]^{4}\\[0.45em]
&\quad =5(1-y_1)^{4},\ \ 0<y_1<1
\end{aligned}
$$
...
$$
\Longrightarrow Y_1\sim\mathrm{Beta}(1,5)
$$

<!-- ref-point: 待第四章的貝塔分配主題發布後，將本篇首次出現的 Beta 分配名稱改為指向該處的站內連結。 -->
...
\begin{aligned}
&f_{\sssig Y_n}(y_n)=5\,f_{\sssig X}(y_n)\bigl[F_{\sssig X}(y_n)\bigr]^{5-1}\\[0.45em]
&\quad =5\times 1\times\bigl[y_n\bigr]^{4}\\[0.45em]
&\quad =5y_n^{4},\ \ 0<y_n<1
\end{aligned}
$$
...
$$
\Longrightarrow Y_n\sim\mathrm{Beta}(5,1)
$$
...
$$
\begin{aligned}
&f_{\sssig Y_1Y_5}(y_1,y_5)\\[0.45em]
&\quad =\frac{5!}{(5-2)!}f_{\sssig X}(y_1)f_{\sssig X}(y_5)\\[0.2em]
&\qquad\quad\times\bigl[F_{\sssig X}(y_5)-F_{\sssig X}(y_1)\bigr]^{3}\\[0.45em]
&\quad =20(y_5-y_1)^{3},\ \ 0<y_1<y_5<1
\end{aligned}
$$
...
&\quad =\bigl[20r^{3}w\bigr]_{0}^{1-r}\\[0.45em]
&\quad =20r^{3}(1-r),\ \ 0<r<1
\end{aligned}
$$
...
\begin{aligned}
&F_{\sssig Y}(y)=\mathbb{P}(Y\leqslant y)=1-\mathbb{P}(Y>y)\\[0.45em]
&\quad =1-\mathbb{P}(X_1>y,X_2>y,X_3>y)\\[0.45em]
&\quad =1-\mathbb{P}(X_1>y)\mathbb{P}(X_2>y)\mathbb{P}(X_3>y)\\[0.45em]
&\quad =1-\bigl[1-\mathbb{P}(X_1\leqslant y)\bigr]^{3}\\[0.45em]
&\quad =1-\bigl[1-F_{\sssig X_1}(y)\bigr]^{3}\\[0.45em]
&\quad =
\left\lbrace
\begin{array}{c@{\quad}l}
0, & y\leqslant 0\\[0.5em]
...
<!-- ref-point: 待第四章的指數分配主題發布後，將本題的 Exp 分配名稱改為指向該處的站內連結。 -->
...
&\quad =\mathbb{P}(X_1\leqslant y,X_2\leqslant y,X_3\leqslant y)\\[0.45em]
&\quad =\mathbb{P}(X_1\leqslant y)\mathbb{P}(X_2\leqslant y)\mathbb{P}(X_3\leqslant y)\\[0.45em]
&\quad =\bigl[\mathbb{P}(X_1\leqslant y)\bigr]^{3}=\bigl[F_{\sssig X_1}(y)\bigr]^{3}\\[0.45em]
&\quad =
\left\lbrace
\begin{array}{c@{\quad}l}
0, & y\leqslant 0\\[0.5em]
...
$$
\begin{aligned}
&F_{\sssig Y_i}(y_i)\\[0.45em]
&\quad =\sum_{k=i}^{n}\binom{n}{k}\bigl[F_{\sssig X}(y_i)\bigr]^{k}\\[0.2em]
&\qquad\quad\times\bigl[1-F_{\sssig X}(y_i)\bigr]^{n-k}
\end{aligned}
$$
</div>
  </li>
...
$$
\begin{aligned}
&F_{\sssig Y_1Y_n}(y_1,y_n)\\[0.45em]
&\quad =\bigl[F_{\sssig X}(y_n)\bigr]^{n}\\[0.45em]
...
&\quad =\mathbb{P}(\text{至少 }i\text{ 個 }X\text{ 比 }y_i\text{ 小})\\[0.45em]
&\quad =\sum_{k=i}^{n}\binom{n}{k}\bigl[F_{\sssig X}(y_i)\bigr]^{k}\\[0.2em]
&\qquad\quad\times\bigl[1-F_{\sssig X}(y_i)\bigr]^{n-k}
\end{aligned}
$$
...
\begin{aligned}
&F_{\sssig Y_1Y_n}(y_1,y_n)\\[0.45em]
&\quad =\mathbb{P}(Y_1\leqslant y_1,Y_n\leqslant y_n)\\[0.45em]
&\quad =\mathbb{P}(Y_n\leqslant y_n)-\mathbb{P}(Y_1>y_1,Y_n\leqslant y_n)\\[0.45em]
&\quad =\bigl[F_{\sssig X}(y_n)\bigr]^{n}\\[0.45em]
&\qquad -\bigl[F_{\sssig X}(y_n)-F_{\sssig X}(y_1)\bigr]^{n}
\end{aligned}
$$
...
&\quad =1-\left(\frac{\,6-y_1\,}{6}\right)^{5}\\[0.45em]
&\qquad y_1=1,2,3,4,5,6
\end{aligned}
$$
...
\begin{aligned}
&p_{\sssig Y_1}(y_1)=F_{\sssig Y_1}(y_1)-F_{\sssig Y_1}(y_1-1)\\[0.45em]
&\quad =\left(\frac{\,7-y_1\,}{6}\right)^{5}-\left(\frac{\,6-y_1\,}{6}\right)^{5}\\[0.45em]
&\qquad y_1=1,2,3,4,5,6
\end{aligned}
$$
...
\begin{aligned}
&\mathbb{P}(Y\geqslant k)=\mathbb{P}\bigl(\min(X_1,\ldots,X_m)\geqslant k\bigr)\\[0.45em]
&\quad =\mathbb{P}(X_1\geqslant k,\ldots,X_m\geqslant k)\\[0.45em]
&\quad =\mathbb{P}(X_1\geqslant k)\cdots\mathbb{P}(X_m\geqslant k)\\[0.45em]
&\quad =\bigl[\mathbb{P}(X_1\geqslant k)\bigr]^{m}\\[0.45em]
&\quad =\left[\sum_{s=k}^{\infty}\mathbb{P}(X_1=s)\right]^{m}=r_{\sssig k}^{m}
\end{aligned}
$$
...
&\quad =\sum_{k=1}^{\infty}r_{\sssig k}^{m}=\sum_{n=1}^{\infty}r_{\sssig n}^{m}
\end{aligned}
$$
