...
topic: 11
order: 311
permalink: /teaching-topics/ch3-p11-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch3-p10-candidate/)把期望值與變異數推廣到二元隨機變數的函數 <span class="text-nowrap">$g(X,Y)$，</span>其中所使用的機率分配是聯合分配。但在[條件機率質量函數](/teaching-topics/ch3-p07-candidate/#def-conditional-pmf)與[條件機率密度函數](/teaching-topics/ch3-p07-candidate/#def-conditional-pdf)之中，我們已經看過另一種機率分配: 把其中一個變數固定成常數之後所得到的條件分配。
...
$$
\begin{aligned}
&\mathbb{E}\bigl[g(X)\mid Y=y\bigr]\\[0.45em]
&\quad =\int_{x\in\mathcal{R}_{\sssig X}}g(x)\,f_{\sssig X\mid Y}(x\mid y)\,dx
\end{aligned}
$$
...
$$
\begin{aligned}
&\mathbb{E}\bigl[g(Y)\mid X=x\bigr]\\[0.45em]
&\quad =\int_{y\in\mathcal{R}_{\sssig Y}}g(y)\,f_{\sssig Y\mid X}(y\mid x)\,dy
\end{aligned}
$$
...
(1) 條件期望值只不過是**使用了條件分配作為機率分配**而得到的期望值，本質上而言還是一種期望值，故[期望值](/teaching-topics/ch2-p06-candidate/#def-expectation)所具有的各種性質，條件期望值都應順勢繼承。
{: .topic-paren-item}

(2) 由於條件期望值繼承了期望值的所有性質，這當中是否包含我們曾提過的特殊期望值，也就是[**變異數**](/teaching-topics/ch2-p208-candidate/#def-variance)呢？這是當然的，這種變異數稱作**條件變異數**，我們馬上會看到[其定義](#def-conditional-variance)。
{: .topic-paren-item}

(3) 條件期望值有一個一般期望值沒有的特色，是**條件期望值是「給定的條件」的函數**，這一點在稍後提到**[雙重期望值定理](/teaching-topics/ch3-p12-candidate/#thm-double-expectation) <span lang="en">(double expectation theorem)</span>** 時我們會有更詳細的了解。
{: .topic-paren-item}
...
$$
\begin{aligned}
&\mathrm{Var}(X\mid Y=y)\\[0.45em]
&\quad =\mathbb{E}\Bigl(\bigl[X-\mathbb{E}(X\mid Y=y)\bigr]^{2}\Bigm\vert Y=y\Bigr)
\end{aligned}
$$
...
$$
\begin{aligned}
&\mathrm{Var}(Y\mid X=x)\\[0.45em]
&\quad =\mathbb{E}\Bigl(\bigl[Y-\mathbb{E}(Y\mid X=x)\bigr]^{2}\Bigm\vert X=x\Bigr)
\end{aligned}
$$
...
(2) 變異數有[速算公式](/teaching-topics/ch2-p208-candidate/#thm-variance-formula):
{: .topic-paren-item}
...
\begin{aligned}
&\mathrm{Var}(X\mid Y=y)\\[0.45em]
&\quad =\mathbb{E}\Bigl(\bigl[X-\mathbb{E}(X\mid Y=y)\bigr]^{2}\Bigm\vert Y=y\Bigr)\\[0.45em]
&\quad =\mathbb{E}\Bigl(X^{2}-2X\,\mathbb{E}(X\mid Y=y)\\[0.2em]
&\qquad +\bigl[\mathbb{E}(X\mid Y=y)\bigr]^{2}\Bigm\vert Y=y\Bigr)\\[0.45em]
&\quad =\mathbb{E}\bigl(X^{2}\mid Y=y\bigr)\\[0.2em]
&\qquad -2\,\mathbb{E}(X\mid Y=y)\,\mathbb{E}(X\mid Y=y)\\[0.2em]
&\qquad +\bigl[\mathbb{E}(X\mid Y=y)\bigr]^{2}\\[0.45em]
&\quad =\mathbb{E}\bigl(X^{2}\mid Y=y\bigr)-\bigl[\mathbb{E}(X\mid Y=y)\bigr]^{2}
\end{aligned}
$$
...
$$
p_{\sssig XY}(5,10)=0.1\neq p_{\sssig X}(5)\,p_{\sssig Y}(10)=0.4\times 0.3=0.12\ \Longrightarrow\ X\not\indep Y
$$
...
\left\lbrace
\begin{array}{l}
\dfrac{0.2}{0.4}=0.5,\quad x=5\\[0.8em]
\dfrac{0.1}{0.4}=0.25,\\[0.3em]
\quad x=10\ \text{or}\ 15\\[0.8em]
0,\quad \text{o.w.}
\end{array}
\right.
$$
...
$$
\begin{aligned}
&\Longrightarrow\ \mathbb{E}(X\mid Y=20)\\[0.2em]
&\quad =5\times 0.5+10\times 0.25\\[0.2em]
&\qquad +15\times 0.25=8.75\\[0.45em]
&\mathbb{E}\bigl(X^{2}\mid Y=20\bigr)\\[0.2em]
&\quad =5^{2}\times 0.5+10^{2}\times 0.25\\[0.2em]
...
$$
\begin{aligned}
&\Longrightarrow\ \mathrm{Var}(X\mid Y=20)\\[0.45em]
&\quad =\mathbb{E}\bigl(X^{2}\mid Y=20\bigr)\\[0.2em]
&\qquad -\bigl[\mathbb{E}(X\mid Y=20)\bigr]^{2}\\[0.45em]
&\quad =93.75-(8.75)^{2}=17.1875
\end{aligned}
$$
...
&=\Bigl[\frac{c}{6}x^{3}\Bigr]_{0}^{2}=\frac{4}{3}c\ \Longrightarrow\ c=\frac{3}{\,4\,}
\end{aligned}
$$
...
&=\int_{0}^{2}\frac{c}{2}x^{2}\,dx=\Bigl[\frac{c}{6}x^{3}\Bigr]_{0}^{2}=\frac{4}{3}c\\[0.45em]
...
&\quad =\Bigl[\frac{2}{3}y^{3}\Bigr]_{0}^{1}=\frac{2}{\,3\,}\\[0.9em]
&\mathbb{E}(X\mid Y=1)=\int_{1}^{2}x\,\frac{1}{2-1}\,dx\\[0.45em]
&\quad =\Bigl[\frac{1}{2}x^{2}\Bigr]_{1}^{2}=\frac{3}{\,2\,}
\end{aligned}
$$
...
\begin{array}{l}
x+y,\\[0.3em]
\quad 0\leqslant x\leqslant 1,\ 0\leqslant y\leqslant 1\\[0.5em]
0,\quad \text{o.w.}
\end{array}
\right.
$$
...
&\quad =x+\frac{1}{\,2\,},\ 0\leqslant x\leqslant 1
\end{aligned}
$$
...
&\quad =\frac{\,\frac{x}{2}+\frac{1}{3}\,}{x+\frac{1}{2}},\ 0\leqslant x\leqslant 1\\[0.9em]
&\mathbb{E}\bigl(Y^{2}\mid X=x\bigr)=\int_{0}^{1}y^{2}\,\frac{x+y}{\,x+\frac{1}{2}\,}\,dy\\[0.45em]
&\quad =\frac{\,\frac{x}{3}+\frac{1}{4}\,}{x+\frac{1}{2}},\ 0\leqslant x\leqslant 1
\end{aligned}
$$
...
\begin{aligned}
&\mathrm{Var}(Y\mid X=x)\\[0.45em]
&\quad =\mathbb{E}\bigl(Y^{2}\mid X=x\bigr)-\bigl[\mathbb{E}(Y\mid X=x)\bigr]^{2}\\[0.45em]
&\quad =\frac{\,6x^{2}+6x+1\,}{18(2x+1)^{2}},\ 0\leqslant x\leqslant 1
\end{aligned}
$$
...
$$
\begin{aligned}
&\mathrm{Var}(2Y+3X+4\mid X=x)\\[0.45em]
&\quad =4\,\mathrm{Var}(Y\mid X=x)\\[0.45em]
&\quad =\frac{\,12x^{2}+12x+2\,}{9(2x+1)^{2}},\ 0\leqslant x\leqslant 1
\end{aligned}
$$
...
[下一篇](/teaching-topics/ch3-p12-candidate/)將由條件期望值是「給定的條件」的函數這一點出發，介紹雙重期望值定理。
