...
date: 2026-05-05
published: true
excerpt: "公理化機率給出合法機率函數的條件，卻不直接說明機率應該怎麼算。本篇以古典機率、計數測度、幾何機率、客觀機率與主觀機率說明不同的指定方式。"
---

[上一篇文章](/teaching-topics/random-experiments-sample-space-events/)先把隨機實驗、樣本空間與事件整理出來。樣本空間 $S$ 說明所有可能結果，事件則是我們關心的結果集合。

接下來有一個自然問題。事件的機率到底怎麼指定？

最熟悉的情形是古典機率。若樣本點有限且均等可能，事件機率可由有利樣本點個數除以總樣本點個數得到。若樣本空間是幾何範圍，則可改由長度、面積或體積的比例指定機率。這些做法都在處理同一個問題，也就是如何為事件指定合理的機率數值。
...
上一篇處理前兩層的直覺，本篇處理第三層，也就是在不同情境下，如何指定機率函數 $\mathbb{P}$ 的數值。公理化機率會在下一篇補上合法性條件。

<div class="topic-box topic-box--note" markdown="1">
<div class="topic-box__label">閱讀提示</div>

本篇不把所有排列組合技巧一次講完，而是先說明它們在機率論中的位置。排列、組合、長度、面積、相對次數或主觀判斷，都可以是指定機率的來源；但指定完成後，它們仍必須回到 Kolmogorov 公理之下，成為一個合法的機率函數。
</div>
...
<div class="topic-box__label">Example 1.3 (Two Dice)</div>
...
在一維中，$m(\cdot)$ 可以是長度；在二維中，可以是面積；在三維中，可以是體積。它背後仍然是一個「均勻」的想法。若落點在整個區域中沒有偏向任何特定位置，事件的機率就由該事件所佔的幾何比例決定。

<div class="topic-box topic-box--example" markdown="1">
<div class="topic-box__label">Example 1.4 (Geometric Probability by Area)</div>
...
這幾種機率看起來來源不同，但它們在現代機率論中扮演同一個角色，也就是提供一個候選的機率函數。
...
Kolmogorov 公理不說明機率要怎麼算；它要求算出來的東西必須像一個機率函數。不同情境下，指定 $\mathbb{P}$ 的方式可能不同。
...
因此，機率計算並不是單一公式，而是一個建模選擇。選定模型後，還要說明哪些事件可以被賦予機率，以及機率函數至少要滿足哪些共同條件。這會導向[事件集合族與 $\sigma$-域](/teaching-topics/event-families-sigma-fields/)。
