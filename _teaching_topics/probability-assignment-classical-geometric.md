...
但這裡仍有一個自然問題。$\mathbb{P}$ 到底怎麼來？

柯爾莫哥洛夫機率公理 (Kolmogorov axioms) 並沒有說明機率應該如何被計算。它要求的是，不論用什麼方式指定機率，最後得到的函數都必須滿足非負性、歸一性與可數可加性。換句話說，公理像是驗收規格；真正的建模工作，還需要說明機率數值從哪裡來。
...
主題二處理的是前兩層與合法性條件。本篇處理第三層，也就是在不同情境下，如何指定機率函數 $\mathbb{P}$ 的數值。
