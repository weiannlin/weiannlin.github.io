...
$$
\begin{aligned}
&\mathbb{P}(a<X<b,\ c<Y<d)\\[0.45em]
&\quad =\int_{c}^{d}\!\!\int_{a}^{b}f_{\sssig XY}(x,y)\,dx\,dy\\[0.45em]
&\quad =\int_{c}^{d}\!\!\int_{a}^{b}f_{\sssig X}(x)\,f_{\sssig Y}(y)\,dx\,dy\\[0.45em]
&\quad =\int_{a}^{b}f_{\sssig X}(x)\,dx\int_{c}^{d}f_{\sssig Y}(y)\,dy\\[0.45em]
&\quad =\mathbb{P}(a<X<b)\,\mathbb{P}(c<Y<d)
\end{aligned}
$$
...
$$
f_{\sssig XY}(x,y)\neq f_{\sssig X}(x)\,f_{\sssig Y}(y)\ \Longleftrightarrow\ X\not\indep Y
$$
...
$$
\begin{gathered}
f_{\sssig XY}(x,y)\neq f_{\sssig X}(x)\,f_{\sssig Y}(y)\\[0.45em]
...
$$
p_{\sssig X_1X_2}(1,1)=0.2\neq p_{\sssig X_1}(1)\,p_{\sssig X_2}(1)=0.4\times 0.4=0.16\ \Longleftrightarrow\ X_1\not\indep X_2
$$
...
&\quad \neq p_{\sssig X_1}(1)\,p_{\sssig X_2}(1)=0.4\times 0.4=0.16\\[0.45em]
&\quad \Longleftrightarrow\ X_1\not\indep X_2
\end{aligned}
$$
...
$$
\alpha=\beta\ \Longrightarrow\ \theta=\frac{\,\alpha\,}{\,\beta\,}=1
$$
...
&\quad =\frac{\mathbb{P}(Y=1\mid X=1)\,\mathbb{P}(Y=0\mid X=0)}{\mathbb{P}(Y=0\mid X=1)\,\mathbb{P}(Y=1\mid X=0)}\\[0.55em]
&\quad =\frac{\dfrac{\mathbb{P}(Y=1,X=1)}{\mathbb{P}(X=1)}\,\dfrac{\mathbb{P}(Y=0,X=0)}{\mathbb{P}(X=0)}}{\dfrac{\mathbb{P}(Y=0,X=1)}{\mathbb{P}(X=1)}\,\dfrac{\mathbb{P}(Y=1,X=0)}{\mathbb{P}(X=0)}}\\[0.55em]
&\quad =\frac{\mathbb{P}(X=1,Y=1)\,\mathbb{P}(X=0,Y=0)}{\mathbb{P}(X=1,Y=0)\,\mathbb{P}(X=0,Y=1)}\\[0.55em]
&\quad =\frac{\dfrac{\mathbb{P}(X=1,Y=1)}{\mathbb{P}(Y=1)}\,\dfrac{\mathbb{P}(X=0,Y=0)}{\mathbb{P}(Y=0)}}{\dfrac{\mathbb{P}(X=1,Y=0)}{\mathbb{P}(Y=0)}\,\dfrac{\mathbb{P}(X=0,Y=1)}{\mathbb{P}(Y=1)}}\\[0.55em]
&\quad =\frac{\mathbb{P}(X=1\mid Y=1)\,\mathbb{P}(X=0\mid Y=0)}{\mathbb{P}(X=0\mid Y=1)\,\mathbb{P}(X=1\mid Y=0)}
\end{aligned}
$$
...
\begin{aligned}
&p_{1\bullet}\,p_{\bullet 1}=(p_{10}+p_{11})(p_{01}+p_{11})\\[0.45em]
&\quad =p_{10}\,p_{01}+p_{10}\,p_{11}\\[0.45em]
&\qquad +p_{11}\,p_{01}+p_{11}\,p_{11}\\[0.45em]
&\quad =p_{11}\,p_{00}+p_{10}\,p_{11}\\[0.45em]
&\qquad +p_{11}\,p_{01}+p_{11}\,p_{11}\\[0.45em]
&\quad =p_{11}\,(p_{00}+p_{10}+p_{01}+p_{11})=p_{11}
\end{aligned}
$$
...
\begin{aligned}
&\lbrace\,(x,y,z)\mid 0<x<y<1,\ 0<z<2\,\rbrace\\[0.45em]
&\quad =\lbrace\,(x,y)\mid 0<x<y<1\,\rbrace\\[0.45em]
...
<!-- ref-point: 待第四章的均勻分配主題發布後，將本段的「均勻分配」改為指向該篇的站內連結。 -->
為一種均勻分配 <span lang="en">(uniform distribution)</span>，故可知 $\mathbb{P}(4X>Y)$ 即為下圖範圍在值域中的面積佔比

<!-- fig-pending: independence-range
     Fig. 3.15，對應書稿 mathstatch3.tex 第 1834 至 1871 行的 tikzpicture，單一面板。
     這張圖畫的是本題所求 $\mathbb{P}(4X>Y)$ 的範圍在聯合值域中所佔的部分:
     第一象限中畫兩條由原點出發的直線，一條是 $x=y$ (書稿 \normaltwo，斜率 1)，
     一條是 $4x=y$ (書稿 \normalthree，斜率 4)；兩線與水平虛線 $y=1$ 之間、
     即 $4x>y$ 且 $x<y$ 的那一塊填色 (書稿用 gray 加 opacity 0.2，
     網頁改 journalaccent 加 0.15)。
     標示: 斜率 1 的直線在其上端右側標 $x=y$；斜率 4 的直線在其上端上方標 $4x=y$；
     由 $(1,1)$ 往左拉一條水平虛線到縱軸，虛線左端標 $y=1$；
     橫軸右端標 $x$、縱軸上端標 $y$，兩軸均帶箭頭。
     檔名 independence-range.svg，anchor 取 #fig-independence-range。
     圖畫好之後，上一段的「下圖」改為指向該 anchor 的 Fig. 3.15 連結。
-->
