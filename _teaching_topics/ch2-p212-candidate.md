...
topic: 12
order: 212
permalink: /teaching-topics/ch2-p212-candidate/
date: 2026-08-06
published: false
...
[上一篇](/teaching-topics/ch2-p211-candidate/)介紹中位數，它把一個分配切成機率各佔一半的前後兩段。這一刀切在正中央，但正中央並不是唯一值得關心的位置: 我們也常想知道有四分之一的機率落在哪一個點以下，或是把一個分配均分成十份、一百份之後，各個分界點分別落在哪裡。
...
令 $X$ 為一隨機變數且 $0<p<1$，若 $x\_{\sssig p}\in\mathbb{R}$ 滿足
...
<figure id="fig-quantile-intuition" class="topic-figure topic-figure--wide">
  ...
  <figcaption><span class="topic-figure__label">Fig. 2.15.</span> $p$-分位數 $x_{\sssig p}$ 把分配切成前後兩段: 界線左側的面積為 $p$、右側的面積為 $1-p$。圖中的密度曲線為 $\mathrm{Gamma}(4,1.7)$，界線取 $p=0.25$，即 $x_{0.25}\fallingdotseq1.491$，也就是 <a href="#fig-kth-q-quantile">Fig. 2.16</a> 的 <span class="text-nowrap">$q_{1}$，</span>兩張圖可以對著看。曲線只畫到 $x=6.4$ 為止，右側實際畫出的面積因而 $\fallingdotseq0.7446$，短少的部分 $\fallingdotseq0.005380$，落在繪製範圍以外的右尾。</figcaption>
</figure>
...
<figure id="fig-kth-q-quantile" class="topic-figure topic-figure--wide">
  ...
  <figcaption><span class="topic-figure__label">Fig. 2.16.</span> 取 $q=4$ 時，三個分位數 $q_{1}$、$q_{2}$、$q_{3}$ 把整個分配均分為四個等份，每一份的機率各為 $\frac{1}{4}$。圖中的密度曲線為 $\mathrm{Gamma}(4,1.7)$，三個分位數依序為 <span class="text-nowrap">$q_{1}\fallingdotseq1.491$、</span><span class="text-nowrap">$q_{2}\fallingdotseq2.160$、</span>$q_{3}\fallingdotseq3.006$。曲線只畫到 $x=6.4$ 為止，最右一塊實際畫出的面積因而 $\fallingdotseq0.2446$，短少的部分 $\fallingdotseq0.005380$，落在繪製範圍以外的右尾。</figcaption>
</figure>
...
[Definition 2.10](/teaching-topics/ch2-p211-candidate/#def-median) 提過中位數未必唯一，下面就是一個中位數不唯一的例子。
...
&\quad =\int_{0}^{2}\frac{1}{\,4\,}\,dx=\frac{1}{\,2\,}
\end{aligned}
$$
...
&\quad =1-F_{\sssig X}(a)=1-F_{\sssig X}(2)\\[0.45em]
&\quad =\int_{4}^{6}\frac{1}{\,4\,}\,dx=\frac{1}{\,2\,}
\end{aligned}
$$
...
(4) 由[第 (1) 小題](/teaching-topics/ch2-p05-candidate/#ex-component-lifetime)已求得 $Y$ 的 cdf 為
{: .topic-paren-item}
