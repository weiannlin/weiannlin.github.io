...
對冪次 $s^k$ 微分一次會產生 $k s^{k-1}$；連續微分 $r$ 次，則會產生由 $k$ 向下相乘的係數。令
...
對 $0\leqslant s<1$，可在冪級數的收斂區間內逐項微分，得到
...
除以 $k!$ 即得所求。$k=0$ 時則由 $G_X(0)=p_X(0)$ 直接成立。
...
這些等式都帶有定義域條件。$G_X(s)$ 在 $\lvert s\rvert\leqslant 1$ 必定存在，不表示 $G_X(e^t)$ 對每個正的 $t$ 都有限；後者正是 MGF 可能發散的原因。
