...
topic: 22
order: 222
permalink: /teaching-topics/ch2-p222-candidate/
date: 2026-08-06
published: false
...
[上一篇](/teaching-topics/ch2-p221-candidate/)由凸函數與凹函數的定義出發，得到延森不等式，把一個隨機變數經過函數轉換前後，兩個期望值的大小關係定了下來。機率不等式這一小節到此告一段落。

本篇改由另一個方向作判斷。若已經知道一個分配的形狀大致呈鐘形，我們可以不經由不等式，直接說出[期望值](/teaching-topics/ch2-p06-candidate/#def-expectation)左右一個、兩個與三個[標準差](/teaching-topics/ch2-p209-candidate/#def-standard-deviation)之內各涵蓋多少機率。以下先說明什麼是經驗法則，接著給出鐘形分配經驗法則的三個機率與兩點使用上的提醒，最後以一張圖把這三個區間的涵蓋比例畫出來。

## 經驗法則

機率不等式在僅有少量的資訊 (例如僅有期望值或[變異數](/teaching-topics/ch2-p208-candidate/#def-variance)) 下，能夠給出一個大致的機率範圍。稍早介紹的[馬可夫不等式](/teaching-topics/ch2-p219-candidate/#thm-markov)、[柴比雪夫不等式](/teaching-topics/ch2-p219-candidate/#thm-chebyshev)、[車諾夫不等式](/teaching-topics/ch2-p220-candidate/#thm-chernoff)與[延森不等式](/teaching-topics/ch2-p221-candidate/#thm-jensen)，都是只憑少量的資訊便替某個機率或某個期望值定出界限的結果。然而根據過往的經驗，我們亦能夠給出一些有用的判斷，這種方法稱作**經驗法則 (empirical rule 或 rule of thumb)**。
...
上列第 (2) 點所說的「從常態分配而來」，具體的算法如下。$X$ 確實服從常態分配時，令 $Z=\frac{\,X-\mu_{\sssig X}\,}{\sigma_{\sssig X}}$，則 $Z$ 服從**標準常態分配 <span lang="en">(standard normal distribution)</span>**；以 $\Phi$ 表示 $Z$ 的 [cdf](/teaching-topics/ch2-p02-candidate/#def-cdf)，則對任意 $k>0$ 皆有
...
$$
\begin{aligned}
&\mathbb{P}\bigl(\lvert X-\mu_{\sssig X}\rvert<k\,\sigma_{\sssig X}\bigr)\\[0.45em]
&\quad =\mathbb{P}\bigl(\lvert Z\rvert<k\bigr)=\Phi(k)-\Phi(-k)\\[0.45em]
&\quad =2\,\Phi(k)-1
\end{aligned}
$$
...
$Z$ 為連續型隨機變數，單點機率為 $0$，故第二個等號中的嚴格不等號不影響機率；末一個等號則用到標準常態分配對 $0$ 的對稱性，即 $\Phi(-k)=1-\Phi(k)$。以 $k=1,2,3$ 代入 $2\,\Phi(k)-1$，四捨五入至小數第四位依序為 $0.6827$、$0.9545$ 與 $0.9973$，這正是 [Theorem 2.37](#thm-empirical-rule) 三款的三個數值。$\Phi$ 由標準常態密度的積分定義，沒有以初等函數寫出的表示式，三個數值須以數值方法求得，本篇不另作證明；常態分配與 $\Phi$ 的完整介紹留待後面的主題。

<!-- ref-point: 待常態分配主題發布後，將本則 Note 的「常態分配」「標準常態分配」與「後面的主題」改為指向該篇文章的站內連結。 -->
...
使用時有兩點要留意。其一，鐘形與土丘形只是對外觀的概括稱呼，不是某一種確切的分配，因此這三個機率能不能用，取決於該分配與常態分配的接近程度: 隨機變數確實服從常態分配時相當準確，只是外觀像鐘則未必。其二，三個數值都是近似值，不宜當成精確的機率。[下一篇](/teaching-topics/ch2-p223-candidate/)轉入隨機變數的函數轉換，說明已知 $X$ 的機率分配時，如何求出 $Y=g(X)$ 的機率分配，並以數道例題示範離散型與連續型的作法。
