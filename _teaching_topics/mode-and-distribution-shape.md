...
本篇討論另一個常見的中央趨勢量數，也就是**眾數 (mode)**。對離散型隨機變數而言，眾數是機率質量函數達到最大值的位置；對連續型隨機變數而言，眾數是密度函數的局部高峰位置。同時，我們也會開始從中央趨勢量數走向分配形狀的觀察，並為後面討論偏態與峰態作準備。

## 眾數的定義

離散型隨機變數的眾數，是使 pmf 達到最大值的可能取值。若 $X$ 具有 pdf，則眾數是密度函數的局部最大位置。
...
若 $X$ 具有 pdf $f_X$，且對 $m_o\in\mathcal{R}_X$，存在 $\delta>0$，使得

$$
f_X(m_o)\geqslant f_X(x),
\qquad
\forall x\in\mathcal{R}_X\cap(m_o-\delta,m_o+\delta)
$$

則亦稱 $m_o$ 為 $X$ 的眾數。換言之，連續型眾數是密度函數的**局部最大位置 (local maximum)**。
...
雖然樣本眾數與母體眾數不是同一個定義，但背後觀念相同。例如一組資料為 $2,2,2,3,4$，其中 $2$ 出現最多次，所以樣本眾數為 $2$。若把這組資料轉換成經驗機率分配，則
...
因此，在這個經驗機率分配中，$2$ 也正是機率最大的點。換句話說，樣本眾數是資料中出現最多次的觀察值；母體眾數則依分配型態而定，離散型取 pmf 的全域最大位置，連續型取 pdf 的局部最大位置。
</div>

<div class="topic-box topic-box--note" markdown="1">
<div class="topic-box__label">Note</div>

離散型眾數可以解釋為機率最大的可能取值。連續型隨機變數的單點機率皆為 $0$，因此不能把眾數解釋成單點機率最大的位置；它表示密度在該處達到局部最大值，也就是機率在該位置附近較為集中。
</div>

## 眾數未必唯一

眾數和中位數一樣，未必唯一。若密度函數在 $\mathcal{R}_X$ 內沒有任何局部最大位置，連續型眾數便不存在。
...
若一個分配只有一個高峰，稱為**單峰分配 (unimodal distribution)**。若有兩個高峰，稱為**雙峰分配 (bimodal distribution)**。若有多個高峰，則稱為**多峰分配 (multimodal distribution)**。在這類圖形描述中，連續型分配的高峰是 pdf 的局部最大位置；離散型分配則依 pmf 圖形中相鄰取值的高低辨認高峰。

對離散型分配而言，[Definition 2.13](#definition-213) 以 pmf 的全域最大位置定義眾數，因此較低的圖形高峰未必是眾數。對連續型分配而言，眾數就是 pdf 的局部最大位置，因此單峰、雙峰與多峰可依局部高峰的個數區分；各峰高度不必相同。
...
| 眾數 | $m_o$ | 離散型取 pmf 的全域最大位置；連續型取 pdf 的局部最大位置 |
...
這裡的比較要注意範圍。期望值、中位數與眾數的靈敏程度，是在討論尾端或極端值如何拉動中央趨勢量數；它不是說眾數在任何情形下都最穩定。眾數只看 pmf 的最大位置或 pdf 的局部高峰位置，因此它對這些位置附近的變化反而可能很敏感。
...
離散型眾數使 pmf 達到最大值，連續型眾數則是 pdf 的局部最大位置，均記作 $m_o$。離散型眾數可解釋為最可能取到的值；連續型眾數表示密度在該處形成局部高峰。眾數可能不唯一，也可能不存在。

期望值、中位數與眾數分別從加權平均、累積機率與 pmf 或 pdf 的高峰位置描述中央趨勢。在有偏分配中，期望值通常最容易被尾端極端值拉動，中位數次之，眾數則多半反映主要高峰所在。下一篇[偏態與峰態](/teaching-topics/skewness-and-kurtosis/)會把這些圖形觀察轉成可計算的形狀量數。
