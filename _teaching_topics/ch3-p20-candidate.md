...
topic: 20
order: 320
permalink: /teaching-topics/ch3-p20-candidate/
date: 2026-08-13
published: false
...
[上一篇](/teaching-topics/ch3-p19-candidate/)以 [Theorem 3.16](/teaching-topics/ch3-p19-candidate/#thm-covar-proper2) 給出線性組合的變異數展開式，並在最後指出，多元隨機變數的表示與運算會隨著變數個數增加而越來越複雜，因此在多變量分析與迴歸分析的領域中，我們更常改以向量的形式來處理一組多元隨機變數。

本篇即由此出發，先以 [Definition 3.17](#def-covar-matrix) 把所有變數之間的變異數與共變異數收在同一個矩陣裡，再以 [Definition 3.18](#def-var-of-r-vvec) 說明這個矩陣其實就是隨機向量的變異數，兩者事實上是同一件事情。接著介紹隨機向量在向量與矩陣上的運算: [Theorem 3.17](#thm-vector-operation) 處理常數向量與隨機向量的乘積，其中變異數的結果稱為二次形式，並由此證明共變異數矩陣必為正半定矩陣；[Theorem 3.18](#thm-matrix-operation) 則把常數向量換成常數矩陣，所得的結果是一個維度比 $1$ 還高的隨機向量。最後以一道二元常態的例題示範這些結果在計算上的用法。
...
(2) 由[共變異數的對稱性](/teaching-topics/ch3-p18-candidate/#thm-covar-proper)，我們可以發現其實 $\sigma_{\sssig XY} = \sigma_{\sssig YX},$ $\sigma_{\sssig XZ} = \sigma_{\sssig ZX}$ 及 <span class="text-nowrap">$\sigma_{\sssig YZ} = \sigma_{\sssig ZY}$，</span>換言之，$\mathbf{\Sigma}$ 是一個對稱矩陣 <span lang="en">(symmetric matrix)</span>。
{: .topic-paren-item}

另外，由於我們曾經知道[變異數非負](/teaching-topics/ch2-p208-candidate/#thm-variance-properties)的特色，故一個合法的共變異數矩陣的主對角線 <span lang="en">(main diagonal)</span> 不應出現負數。例如以下的矩陣即不為合法的共變異數矩陣:
{: .topic-paren-cont}
...
讀者或許好奇，前二個矩陣不能當作共變異數矩陣的原因很明顯，但第三個矩陣為何不能作為共變異數矩陣呢？這個原因在稍後我們介紹[**相關係數 <span lang="en">(correlation coefficient)</span>**](/teaching-topics/ch3-p21-candidate/#def-corr)的時候會談到。
...
=\begin{bmatrix} \mathbb{E}(X_1)\\ \vdots\\ \mathbb{E}(X_n) \end{bmatrix}=\begin{bmatrix} \mu_1\\ \vdots\\ \mu_n \end{bmatrix}
\end{gathered}
$$

</div>

表示 $\boldsymbol{X}$ 的期望值。
...
=\mathbb{E}\begin{bmatrix} (X_1-\mu_1)(X_1-\mu_1) & \cdots & (X_1-\mu_1)(X_n-\mu_n)\\ \vdots & \ddots & \vdots\\ (X_n-\mu_n)(X_1-\mu_1) & \cdots & (X_n-\mu_n)(X_n-\mu_n) \end{bmatrix}\\[0.55em]
=\begin{bmatrix} \mathbb{E}\bigl[(X_1-\mu_1)(X_1-\mu_1)\bigr] & \cdots & \mathbb{E}\bigl[(X_1-\mu_1)(X_n-\mu_n)\bigr]\\ \vdots & \ddots & \vdots\\ \mathbb{E}\bigl[(X_n-\mu_n)(X_1-\mu_1)\bigr] & \cdots & \mathbb{E}\bigl[(X_n-\mu_n)(X_n-\mu_n)\bigr] \end{bmatrix}\\[0.55em]
=\begin{bmatrix} \sigma_{1}^{2} & \sigma_{12} & \cdots & \sigma_{1n}\\ \sigma_{21} & \sigma_{2}^{2} & \cdots & \sigma_{2n}\\ \vdots & \vdots & \ddots & \vdots\\ \sigma_{n1} & \sigma_{n2} & \cdots & \sigma_{n}^{2} \end{bmatrix}
\end{gathered}
$$
...
讀者在此應該特別注意的地方是，與矩陣相關的運算，都可以算是線性 (linear) 的運算，因此如果我們對隨機向量乘上某個向量 <span class="text-nowrap">$\boldsymbol{a}^{\mathrm{T}}$，</span>則其運算完全可以承襲過去我們所學的隨機變數的線性變換性質，包含[期望值的線性轉換](/teaching-topics/ch2-p207-candidate/#thm-expectation-linearity)，與[變異數平方擴充](/teaching-topics/ch2-p208-candidate/#thm-variance-properties)等等特性。我們在此以向量及矩陣的形式呈現。
...
=\begin{bmatrix} a_{11} & \cdots & a_{1n}\\ \vdots & \ddots & \vdots\\ a_{k1} & \cdots & a_{kn} \end{bmatrix}
\end{gathered}
$$
...
=\mathbb{E}\Biggl(\begin{bmatrix} \boldsymbol{a}_1^{\mathrm{T}}\boldsymbol{X}\\ \vdots\\ \boldsymbol{a}_{k}^{\mathrm{T}}\boldsymbol{X} \end{bmatrix}\Biggr)=\begin{bmatrix} \boldsymbol{a}_1^{\mathrm{T}}\boldsymbol{\mu}\\ \vdots\\ \boldsymbol{a}_{k}^{\mathrm{T}}\boldsymbol{\mu} \end{bmatrix}\\[0.55em]
=\begin{bmatrix} \boldsymbol{a}_1^{\mathrm{T}}\\ \vdots\\ \boldsymbol{a}_{k}^{\mathrm{T}} \end{bmatrix}\boldsymbol{\mu}=\mathbf{A}\boldsymbol{\mu}
\end{gathered}
$$
...
=\Bigl[\,3\ \ 2\,\Bigr]\begin{bmatrix} 4 & 2\\ 2 & 3 \end{bmatrix}\begin{bmatrix} 3\\ 2 \end{bmatrix}=72
\end{gathered}
$$
...
讀者目前應該已經能夠理解與掌握[共變異數](/teaching-topics/ch3-p18-candidate/#def-covariance)，但是，作為刻畫 $X$ 與 $Y$ 共變關係的共變異數，卻沒有範圍上的限制，僅知道共同變化的「方向」，不知道「程度」，這該如何是好呢？為了解決這個問題，我們將會介紹[**相關係數**](/teaching-topics/ch3-p21-candidate/#def-corr)。

## 本篇小結

[Definition 3.17](#def-covar-matrix) 把三個隨機變數兩兩之間的變異數與共變異數收在同一個矩陣裡，主對角線上的 $\sigma^{2}_{\sssig X}, \sigma^{2}_{\sssig Y}, \sigma^{2}_{\sssig Z}$ 是各個變數自己的變異數，其餘位置則是變數之間的共變異數，因此知道一個共變異數矩陣，就等於知道所有變數之間的變異數與共變異數。由[共變異數的對稱性](/teaching-topics/ch3-p18-candidate/#thm-covar-proper)可知 $\mathbf{\Sigma}$ 必為對稱矩陣，又由[變異數非負](/teaching-topics/ch2-p208-candidate/#thm-variance-properties)可知主對角線不應出現負數，這兩件事各排除了一類不合法的矩陣；至於主對角線為正、又對稱的矩陣為何仍可能不合法，要等到相關係數才能回答。同樣的定義並不限於三個變數，$n$ 個變數的版本只是把矩陣擴充為 $n \times n$ 而已。
...
[下一篇](/teaching-topics/ch3-p21-candidate/)將介紹相關係數，它把共變異數除以兩個標準差，使共同變化的「程度」也有範圍上的限制。
