...
topic: 9
order: 309
permalink: /teaching-topics/ch3-p09-candidate/
date: 2026-08-12
published: false
...
[上一篇](/teaching-topics/ch3-p08-candidate/)以五道例題示範了條件機率質量函數與條件機率密度函數的求法，並把條件放寬到一整段範圍，得到截尾分配。這些例題所處理的，都是在另一個變數取定某個值或落在某個範圍之後，這個變數的機率分配會變成什麼樣子。

本篇處理的則是相反的情形: 若這個條件完全沒有改變原本的機率分配，也就是條件機率密度函數與邊際機率密度函數一模一樣，那會是什麼狀況。這正是第一章[事件的獨立](/teaching-topics/independence-and-conditional-independence/#definition-117)推廣到隨機變數之後的樣子。在進入獨立的定義之前，我們先把條件機率密度函數的定義移項，得到乘法原理的機率函數版本。
...
=\lbrace\,(x,y)\mid 0<x<y<1\,\rbrace\times\lbrace\,z\mid 0<z<2\,\rbrace
\end{gathered}
$$
...
\times\lbrace\,z\mid 0<z<2\,\rbrace
\end{gathered}
$$
...
[下一篇](/teaching-topics/ch3-p10-candidate/)把期望值推廣到多元隨機變數，並說明兩個隨機變數獨立時，其函數乘積的期望值可以拆成兩個期望值的乘積。
